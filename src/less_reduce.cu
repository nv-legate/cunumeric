...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_lt_reduce_1d(DeferredValue<bool> result, const AccessorRO<T, 1> in1, const AccessorRO<T, 1> in2, const Point<1> origin,
                        const size_t max) {
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_lt_reduce_2d(DeferredValue<bool> result, const AccessorRO<T, 2> in1, const AccessorRO<T, 2> in2, const Point<2> origin,
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_lt_reduce_3d(DeferredValue<bool> result, const AccessorRO<T, 3> in1, const AccessorRO<T, 3> in2, const Point<3> origin,
...
template<typename T>
/*static*/ DeferredValue<bool> LessReducTask<T>::gpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions,
  ...
  switch (dim) {
    case 1: {
      ...
      legate_lt_reduce_1d<T><<<blocks, THREADS_PER_BLOCK>>>(result, in1, in2, rect.lo, volume);
      break;
    }
    case 2: {
      ...
      legate_lt_reduce_2d<T><<<blocks, THREADS_PER_BLOCK>>>(result, in1, in2, rect.lo, Point<1>(pitch), volume);
      break;
    }
    case 3: {
      ...
      legate_lt_reduce_3d<T><<<blocks, THREADS_PER_BLOCK>>>(result, in1, in2, rect.lo, Point<2>(pitch), volume);
      break;
    }
    default:
      assert(false);
...
INSTANTIATE_DEFERRED_VALUE_TASK_VARIANT(LessReducTask, bool, gpu_variant)
