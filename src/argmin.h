...
//   static const int                     REDOP_ID = NUMPY_ARGMIN_REDOP * MAX_TYPE_NUMBER +
//   COMPLEX64_LT;
...
class ArgminReducTask : public NumPyTask<ArgminReducTask<T>> {
 ...
 public:
  static Argval<T> cpu_variant(const Legion::Task* task,
                               const std::vector<Legion::PhysicalRegion>& regions,
                               Legion::Context ctx,
                               Legion::Runtime* runtime);
#ifdef LEGATE_USE_OPENMP
  static Argval<T> omp_variant(const Legion::Task* task,
                               const std::vector<Legion::PhysicalRegion>& regions,
                               Legion::Context ctx,
                               Legion::Runtime* runtime);
#endif
#ifdef LEGATE_USE_CUDA
  static Legion::DeferredReduction<ArgminReduction<T>> gpu_variant(
