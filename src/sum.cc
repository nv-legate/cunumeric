...
namespace numpy {
    ...
    /*static*/ void SumTask<T>::set_layout_constraints(LegateVariant variant, 
                                 TaskLayoutConstraintSet &layout_constraints)
    {
      // Need a reduction accessor for the first region
      layout_constraints.add_layout_constraint(0, 
          Legate::get_reduction_layout(SumReduction<T>::REDOP_ID));
...
template<typename T>
/*static*/ void SumTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                        Runtime* runtime) {
  ...
  switch (dim) {
    ...
    case 2: {
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
          SumReduction<T>::template fold<true /*exclusive*/>(inout[x][y], in[x][y]);
      break;
    }
    case 3: {
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
          for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++)
            SumReduction<T>::template fold<true /*exclusive*/>(inout[x][y][z], in[x][y][z]);
      break;
    }
    default:
      assert(false);
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ void SumTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                        Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
...
template<typename T>
/*static*/ T SumReducTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                          Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim    = derez.unpack_dimension();
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ T SumReducTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                          Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim         = derez.unpack_dimension();
...
#  pragma omp for nowait
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          SumReduction<T>::template fold<true /*exclusive*/>(results[tid], in[x]);
      }
      break;
    }
    case 2: {
...
#  pragma omp for nowait
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
            SumReduction<T>::template fold<true /*exclusive*/>(results[tid], in[x][y]);
      }
      break;
    }
    case 3: {
...
#  pragma omp for nowait
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
            for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++)
              SumReduction<T>::template fold<true /*exclusive*/>(results[tid], in[x][y][z]);
      }
      break;
    }
    default:
...
template<typename T>
/*static*/ void SumRadixTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                             Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
    ...
    case 1: {
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) {
        ...
        for (unsigned idx = 1; idx < num_inputs; idx++)
          SumReduction<T>::template fold<true /*exclusive*/>(val, in[idx][x]);
        out[x] = val;
      }
      break;
    }
    case 2: {
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++) {
          T val = in[0][x][y];
          for (unsigned idx = 1; idx < num_inputs; idx++)
            SumReduction<T>::template fold<true /*exclusive*/>(val, in[idx][x][y]);
          out[x][y] = val;
        }
      break;
    }
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
          for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++) {
            T val = in[0][x][y][z];
            for (unsigned idx = 1; idx < num_inputs; idx++)
              SumReduction<T>::template fold<true /*exclusive*/>(val, in[idx][x][y][z]);
            out[x][y][z] = val;
          }
      break;
    }
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ void SumRadixTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                             Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  assert(task->regions.size() <= MAX_REDUCTION_RADIX);
  const int     radix         = derez.unpack_dimension();
...
template<typename T>
/*static*/ T SumScalarTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                           Runtime* runtime) {
  assert(task->futures.size() == 2);
  T       one = task->futures[0].get_result<T>();
  const T two = task->futures[1].get_result<T>();
  SumReduction<T>::template fold<true /*exclusive*/>(one, two);
  return one;
}

INSTANTIATE_ALL_TASKS(SumTask, static_cast<int>(NumPyOpCode::NUMPY_SUM) * NUMPY_TYPE_OFFSET)
INSTANTIATE_ALL_TASKS(SumReducTask, static_cast<int>(NumPyOpCode::NUMPY_SUM) * NUMPY_TYPE_OFFSET + NUMPY_REDUCTION_VARIANT_OFFSET)
INSTANTIATE_ALL_TASKS(SumRadixTask, static_cast<int>(NumPyOpCode::NUMPY_SUM_RADIX) * NUMPY_TYPE_OFFSET)
INSTANTIATE_ALL_TASKS(SumScalarTask, static_cast<int>(NumPyOpCode::NUMPY_SUM) * NUMPY_TYPE_OFFSET + NUMPY_SCALAR_VARIANT_OFFSET)
...
static void __attribute__((constructor)) register_tasks(void) {
  REGISTER_ALL_TASKS(legate::numpy::SumTask)
  REGISTER_ALL_TASKS_WITH_REDUCTION_RETURN(legate::numpy::SumReducTask, SumReduction)
  REGISTER_ALL_TASKS(legate::numpy::SumRadixTask)
  REGISTER_ALL_TASKS_WITH_RETURN(legate::numpy::SumScalarTask)
}
}    // namespace
