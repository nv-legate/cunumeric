...
template <typename T>
/*static*/ void SumTask<T>::cpu_variant(const Task* task,
...
#ifdef LEGATE_USE_OPENMP
template <typename T>
/*static*/ void SumTask<T>::omp_variant(const Task* task,
                                        const std::vector<PhysicalRegion>& regions,
                                        Context ctx,
                                        Runtime* runtime)
{
...
template <typename T>
/*static*/ T SumReducTask<T>::cpu_variant(const Task* task,
                                          const std::vector<PhysicalRegion>& regions,
                                          Context ctx,
                                          Runtime* runtime)
{
  ...
  switch (dim) {
    case 1: {
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) result += in[x];
      break;
    }
    case 2: {
      const Rect<2> rect = NumPyProjectionFunctor::unpack_shape<2>(task, derez);
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++) result += in[x][y];
      break;
    }
    case 3: {
      const Rect<3> rect = NumPyProjectionFunctor::unpack_shape<3>(task, derez);
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
          for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++) result += in[x][y][z];
      break;
    }
    default: assert(false);
  }
...
#ifdef LEGATE_USE_OPENMP
template <typename T>
/*static*/ T SumReducTask<T>::omp_variant(const Task* task,
                                          const std::vector<PhysicalRegion>& regions,
                                          Context ctx,
                                          Runtime* runtime)
{
...
template <typename T>
/*static*/ void SumRadixTask<T>::cpu_variant(const Task* task,
                                             const std::vector<PhysicalRegion>& regions,
                                             Context ctx,
                                             Runtime* runtime)
{
...
#ifdef LEGATE_USE_OPENMP
template <typename T>
/*static*/ void SumRadixTask<T>::omp_variant(const Task* task,
                                             const std::vector<PhysicalRegion>& regions,
                                             Context ctx,
                                             Runtime* runtime)
{
...
template <typename T>
/*static*/ T SumScalarTask<T>::cpu_variant(const Task* task,
                                           const std::vector<PhysicalRegion>& regions,
                                           Context ctx,
                                           Runtime* runtime)
{
...
INSTANTIATE_ALL_TASKS(SumReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_SUM) * NUMPY_TYPE_OFFSET +
                        NUMPY_REDUCTION_VARIANT_OFFSET)
INSTANTIATE_ALL_TASKS(SumRadixTask,
                      static_cast<int>(NumPyOpCode::NUMPY_SUM_RADIX) * NUMPY_TYPE_OFFSET)
INSTANTIATE_ALL_TASKS(SumScalarTask,
                      static_cast<int>(NumPyOpCode::NUMPY_SUM) * NUMPY_TYPE_OFFSET +
