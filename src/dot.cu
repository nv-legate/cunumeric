...
template<>
/*static*/ void DotTask<__half>::gpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                             Runtime* runtime) {
  ...
  switch (dim) {
    case 1: {
      ...
      if (dim1 == 1) {
        ...
        CHECK_CUBLAS(cublasSgemmEx(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, 1, m, &alpha, in2_ptr, CUDA_R_16F, in2_strides[0],
                                   in1_ptr, CUDA_R_16F, m, &beta, out_ptr, partial ? CUDA_R_32F : CUDA_R_16F, n));
      } else {
        assert(dim1 == 2);
        ...
        // Use SgemmEx here since there is no half precision gemv yet
        CHECK_CUBLAS(cublasSgemmEx(cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, m, 1, n, &alpha, in1_ptr, CUDA_R_16F, in1_strides[0],
                                   in2_ptr, CUDA_R_16F, n, &beta, out_ptr, partial ? CUDA_R_32F : CUDA_R_16F, m));
      }
      break;
    }
    case 2: {
      ...
      CHECK_CUBLAS(cublasSgemmEx(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha, in2_ptr, CUDA_R_16F, in2_strides[0],
                                 in1_ptr, CUDA_R_16F, in1_strides[0], &beta, out_ptr, partial ? CUDA_R_32F : CUDA_R_16F,
                                 out_strides[0]));
      break;
    }
    default:
...
template<>
/*static*/ void DotTask<float>::gpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                            Runtime* runtime) {
  cublasHandle_t cublas_handle = Core::get_cublas();
  // Update the stream because the CUDA hijack can't see inside cuBLAS
  cudaStream_t task_stream;
  ...
  switch (dim) {
    case 1: {
      ...
      if (task->regions[0].privilege == READ_WRITE) {
        const AccessorRW<float, 1> out =
            (extra_dim >= 0)
                ? derez.unpack_accessor_RW<float, 1>(regions[0], out_rect, 1 /*out extram dim*/, task->index_point[extra_dim])
                : derez.unpack_accessor_RW<float, 1>(regions[0], out_rect);
        out_ptr = out.ptr(out_rect);
      } else {
        const AccessorWO<float, 1> out =
      ...
      if (dim1 == 1) {
        ...
        CHECK_CUBLAS(cublasSgemv(cublas_handle, CUBLAS_OP_N, n, m, &alpha, in2_ptr, in2_strides[0], in1_ptr, 1, &beta, out_ptr, 1));
      } else {
        assert(dim1 == 2);
        const Rect<2> in1_rect = NumPyProjectionFunctor::unpack_shape<2>(task, derez);
        if (in1_rect.empty()) return;
        ...
        CHECK_CUBLAS(cublasSgemv(cublas_handle, CUBLAS_OP_T, n, m, &alpha, in1_ptr, in1_strides[0], in2_ptr, 1, &beta, out_ptr, 1));
      }
      break;
    }
    case 2: {
      ...
      // Use the extended sgemm interface so we can use tensor cores
      // if they are available for this matrix shape and GPU
      CHECK_CUBLAS(cublasSgemmEx(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha, in2_ptr, CUDA_R_32F, in2_strides[0],
                                 in1_ptr, CUDA_R_32F, in1_strides[0], &beta, out_ptr, CUDA_R_32F, out_strides[0]));
      break;
    }
    default:
      assert(false);    // we don't support any other updates
...
template<>
/*static*/ void DotTask<double>::gpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                             Runtime* runtime) {
  cublasHandle_t cublas_handle = Core::get_cublas();
  // Update the stream because the CUDA hijack can't see inside cuBLAS
  cudaStream_t task_stream;
  ...
  switch (dim) {
    case 1: {
      ...
      if (dim1 == 1) {
        ...
        CHECK_CUBLAS(cublasDgemv(cublas_handle, CUBLAS_OP_N, n, m, &alpha, in2_ptr, in2_strides[0], in1_ptr, 1, &beta, out_ptr, 1));
      } else {
        assert(dim1 == 2);
        const Rect<2> in1_rect = NumPyProjectionFunctor::unpack_shape<2>(task, derez);
        if (in1_rect.empty()) return;
        ...
        CHECK_CUBLAS(cublasDgemv(cublas_handle, CUBLAS_OP_T, n, m, &alpha, in1_ptr, in1_strides[0], in2_ptr, 1, &beta, out_ptr, 1));
      }
      break;
    }
    case 2: {
      ...
      // order to help cublas think things are column-major
      // effectively we get NxM = NxK * KxM
      CHECK_CUBLAS(cublasDgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, m, k, &alpha, in2_ptr, in2_strides[0], in1_ptr,
                               in1_strides[0], &beta, out_ptr, out_strides[0]));
      break;
    }
    default:
      assert(false);    // we don't support any other updates
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_dot_reduce(const DeferredBuffer<T, 1> buffer, const AccessorRO<T, 1> in1, const AccessorRO<T, 1> in2,
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_buffer_dot_reduce(const DeferredBuffer<T, 1> in, const DeferredBuffer<T, 1> out, const size_t max, const T identity) {
  T            value  = identity;
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_final_dot_reduce(const DeferredBuffer<T, 1> in, const DeferredReduction<SumReduction<T>> out, const size_t max,
                            const T identity) {
  T            value  = identity;
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset < max) value = in.read(offset);
...
/*static*/ DeferredReduction<SumReduction<T>>
    DotReducTask<T>::gpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim = derez.unpack_dimension();
  assert(dim == 1);
  ...
  if (!rect.empty()) {
    ...
    legate_dot_reduce<T><<<blocks, THREADS_PER_BLOCK>>>(bufferA, in1, in2, rect.lo, volume, SumReduction<T>::identity);
    volume = blocks;
  }
  ...
  if (volume > THREADS_PER_BLOCK) {
    ...
    while (volume > THREADS_PER_BLOCK) {
      ...
      if (forward) {
        legate_buffer_dot_reduce<T><<<blocks, THREADS_PER_BLOCK>>>(bufferA, bufferB, volume, SumReduction<T>::identity);
        forward = false;
      } else {
        legate_buffer_dot_reduce<T><<<blocks, THREADS_PER_BLOCK>>>(bufferB, bufferA, volume, SumReduction<T>::identity);
        forward = true;
      }
      volume = blocks;
    }
  ...
  if (volume > 0) legate_final_dot_reduce<T><<<1, THREADS_PER_BLOCK>>>(last, result, volume, SumReduction<T>::identity);
  return result;
}
