...
/*static*/ const Argval<__half> ArgminReduction<__half>::identity = Argval<__half>(MinReduction<__half>::identity);
/*static*/ const Argval<float>  ArgminReduction<float>::identity  = Argval<float>(MinReduction<float>::identity);
/*static*/ const Argval<double> ArgminReduction<double>::identity = Argval<double>(MinReduction<double>::identity);

/*static*/ const Argval<int16_t> ArgminReduction<int16_t>::identity = Argval<int16_t>(MinReduction<int16_t>::identity);
/*static*/ const Argval<int32_t> ArgminReduction<int32_t>::identity = Argval<int32_t>(MinReduction<int32_t>::identity);
/*static*/ const Argval<int64_t> ArgminReduction<int64_t>::identity = Argval<int64_t>(MinReduction<int64_t>::identity);

/*static*/ const Argval<uint16_t> ArgminReduction<uint16_t>::identity = Argval<uint16_t>(MinReduction<uint16_t>::identity);
/*static*/ const Argval<uint32_t> ArgminReduction<uint32_t>::identity = Argval<uint32_t>(MinReduction<uint32_t>::identity);
/*static*/ const Argval<uint64_t> ArgminReduction<uint64_t>::identity = Argval<uint64_t>(MinReduction<uint64_t>::identity);
...
/*static*/ const Argval<bool> ArgminReduction<bool>::identity = Argval<bool>(MinReduction<bool>::identity);

template<typename T>
/*static*/ void ArgminTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                           Runtime* runtime) {
  ...
  switch (init_dim) {
    case 1: {
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        out[x] = ArgminReduction<T>::identity;
      break;
    }
    case 2: {
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
          out[x][y] = ArgminReduction<T>::identity;
      break;
    }
    default:
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ void ArgminTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                           Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          axis         = derez.unpack_dimension();
  const int          collapse_dim = derez.unpack_dimension();
...
template<typename T>
/*static*/ Argval<T> ArgminReducTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                     Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim    = derez.unpack_dimension();
  ...
  switch (dim) {
    ...
    default:
      assert(false);    // shouldn't get any other dimensions here
  }
  return result;
}
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ Argval<T> ArgminReducTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                     Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim    = derez.unpack_dimension();
  Argval<T>          result = ArgminReduction<T>::identity;
...
template<typename T>
/*static*/ void ArgminRadixTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  ...
  switch (dim) {
    case 1: {
      ...
      for (unsigned idx = 1; idx < task->regions.size(); idx++)
        if (task->regions[idx].region.exists())
          in[num_inputs++] = derez.unpack_accessor_RO<Argval<T>, 1>(regions[idx], rect, extra_dim_in, offset + idx - 1);
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) {
        Argval<T> val = in[0][x];
    ...
    case 2: {
      ...
      for (unsigned idx = 1; idx < task->regions.size(); idx++)
        if (task->regions[idx].region.exists())
          in[num_inputs++] = derez.unpack_accessor_RO<Argval<T>, 2>(regions[idx], rect, extra_dim_in, offset + idx - 1);
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++) {
    ...
    case 3: {
      ...
      for (unsigned idx = 1; idx < task->regions.size(); idx++)
        if (task->regions[idx].region.exists())
          in[num_inputs++] = derez.unpack_accessor_RO<Argval<T>, 3>(regions[idx], rect, extra_dim_in, offset + idx - 1);
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
          for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++) {
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ void ArgminRadixTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  assert(task->regions.size() < MAX_REDUCTION_RADIX);
  const int     radix         = derez.unpack_dimension();
...
INSTANTIATE_ALL_TASKS(ArgminReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_ARGMIN) * NUMPY_TYPE_OFFSET + NUMPY_REDUCTION_VARIANT_OFFSET)
INSTANTIATE_ALL_TASKS(ArgminRadixTask, static_cast<int>(NumPyOpCode::NUMPY_ARGMIN_RADIX) * NUMPY_TYPE_OFFSET)
...
static void __attribute__((constructor)) register_tasks(void) {
  ...
  REGISTER_ALL_TASKS_WITH_WRAP_REDUCTION_RETURN(legate::numpy::ArgminReducTask, legate::numpy::Argval,
                                                legate::numpy::ArgminReduction)
