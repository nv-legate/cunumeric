...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM) gpu_broadcast_binary_op(const Args args, const bool dense) {
...
public:
  static_assert(std::is_same<first_argument_type, second_argument_type>::value,
                "BroadcastBinaryOperation currently requires first_argument_type and second_argument_type to be the same type.");
  static const int TASK_ID =
      task_id<BinaryFunction::op_code, NUMPY_BROADCAST_VARIANT_OFFSET, result_type, first_argument_type, second_argument_type>;
...
#  pragma omp parallel for schedule(static)
  ...
  static void dispatch_gpu(const Legion::Task* task, const std::vector<Legion::PhysicalRegion>& regions,
    ...
    gpu_broadcast_binary_op<DIM, BinaryFunction, DeserializedArgs<DIM>><<<blocks, THREADS_PER_BLOCK>>>(args, dense);
  }
