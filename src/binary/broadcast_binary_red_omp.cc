...
#pragma omp parallel for schedule(static)
    for (size_t idx = 0; idx < volume; ++idx)
      if (!func(in1ptr[idx], in2)) result = false;
  } else {
#pragma omp parallel for schedule(static)
    for (size_t idx = 0; idx < volume; ++idx) {
      auto point = pitches.unflatten(idx, rect.lo);
      if (!func(in1[point], in2)) result = false;
    }
  }
...
/*static*/ UntypedScalar BroadcastBinaryRedTask::omp_variant(
  const Task *task, const std::vector<PhysicalRegion> &regions, Context context, Runtime *runtime)
{
  ...
  return reduce_op_dispatch(op_code, omp::BroadcastBinaryRedDispatch{}, shape, in1, in2, args);
}
