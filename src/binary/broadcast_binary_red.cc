                                            ...
                                            bool dense)
{
  ...
  if (dense) {
    ...
    for (size_t idx = 0; idx < volume; ++idx)
      if (!func(in1ptr[idx], in2)) return UntypedScalar(false);
  } else {
    for (size_t idx = 0; idx < volume; ++idx) {
      auto point = pitches.unflatten(idx, rect.lo);
      if (!func(in1[point], in2)) return UntypedScalar(false);
    }
  }
...
/*static*/ UntypedScalar BroadcastBinaryRedTask::cpu_variant(
  const Task *task, const std::vector<PhysicalRegion> &regions, Context context, Runtime *runtime)
{
  ...
  return reduce_op_dispatch(op_code, BroadcastBinaryRedDispatch{}, shape, in1, in2, args);
}
...
{
  BroadcastBinaryRedTask::register_variants_with_return<UntypedScalar, UntypedScalar>();
}
}  // namespace
