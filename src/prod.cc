...
template <typename T>
/*static*/ void ProdTask<T>::cpu_variant(const Task* task,
...
#ifdef LEGATE_USE_OPENMP
template <typename T>
/*static*/ void ProdTask<T>::omp_variant(const Task* task,
                                         const std::vector<PhysicalRegion>& regions,
                                         Context ctx,
                                         Runtime* runtime)
{
...
template <typename T>
/*static*/ T ProdReducTask<T>::cpu_variant(const Task* task,
                                           const std::vector<PhysicalRegion>& regions,
                                           Context ctx,
                                           Runtime* runtime)
{
...
#ifdef LEGATE_USE_OPENMP
template <typename T>
/*static*/ T ProdReducTask<T>::omp_variant(const Task* task,
                                           const std::vector<PhysicalRegion>& regions,
                                           Context ctx,
                                           Runtime* runtime)
{
  ...
  for (int i = 0; i < max_threads; i++) results[i] = ProdReduction<T>::identity;
  switch (dim) {
    case 1: {
      const Rect<1> rect = NumPyProjectionFunctor::unpack_shape<1>(task, derez);
      if (rect.empty()) break;
...
template <typename T>
/*static*/ void ProdRadixTask<T>::cpu_variant(const Task* task,
                                              const std::vector<PhysicalRegion>& regions,
                                              Context ctx,
                                              Runtime* runtime)
{
...
#ifdef LEGATE_USE_OPENMP
template <typename T>
/*static*/ void ProdRadixTask<T>::omp_variant(const Task* task,
                                              const std::vector<PhysicalRegion>& regions,
                                              Context ctx,
                                              Runtime* runtime)
{
...
template <typename T>
/*static*/ T ProdScalarTask<T>::cpu_variant(const Task* task,
                                            const std::vector<PhysicalRegion>& regions,
                                            Context ctx,
                                            Runtime* runtime)
{
...
INSTANTIATE_ALL_TASKS(ProdReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_PROD) * NUMPY_TYPE_OFFSET +
                        NUMPY_REDUCTION_VARIANT_OFFSET)
INSTANTIATE_ALL_TASKS(ProdRadixTask,
                      static_cast<int>(NumPyOpCode::NUMPY_PROD_RADIX) * NUMPY_TYPE_OFFSET)
INSTANTIATE_ALL_TASKS(ProdScalarTask,
                      static_cast<int>(NumPyOpCode::NUMPY_PROD) * NUMPY_TYPE_OFFSET +
