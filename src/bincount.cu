...
template <typename T>
__global__ void legate_bincount_1d(const AccessorWO<uint64_t, 1> out,
                                   ...
                                   const T num_bins)
{
  ...
  for (int bin = threadIdx.x; bin < num_bins; bin += blockDim.x) bins[bin] = 0;
  __syncthreads();
...
template <typename T>
__global__ void legate_bincount_2d(const AccessorWO<uint64_t, 1> out,
...
template <typename T>
__global__ void legate_bincount_3d(const AccessorWO<uint64_t, 1> out,
...
template <typename T>
/*static*/ void BinCountTask<T>::gpu_variant(const Task* task,
  ...
  switch (dim) {
    case 1: {
      ...
      cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &num_ctas, legate_bincount_1d<T>, THREADS_PER_BLOCK, bin_size);
      assert(num_ctas > 0);
      // Launch a kernel with this number of CTAs
      const size_t volume = rect.volume();
      legate_bincount_1d<T>
        <<<num_ctas, THREADS_PER_BLOCK, bin_size>>>(out, in, rect.lo, volume, bin_volume);
      break;
    }
    case 2: {
      ...
      int num_ctas = 0;
      cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &num_ctas, legate_bincount_2d<T>, THREADS_PER_BLOCK, bin_size);
      assert(num_ctas > 0);
      // Launch a kernel with this number of CTAs
      const size_t volume = rect.volume();
      const coord_t pitch = rect.hi[1] - rect.lo[1] + 1;
      legate_bincount_2d<T>
        <<<num_ctas, THREADS_PER_BLOCK, bin_size>>>(out, in, rect.lo, pitch, volume, bin_volume);
      break;
    }
    case 3: {
      ...
      int num_ctas = 0;
      cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &num_ctas, legate_bincount_3d<T>, THREADS_PER_BLOCK, bin_size);
      assert(num_ctas > 0);
      // Launch a kernel with this number of CTAs
      const size_t volume    = rect.volume();
      ...
      legate_bincount_3d<T><<<num_ctas, THREADS_PER_BLOCK, bin_size>>>(
        out, in, rect.lo, Point<2>(pitch), volume, bin_volume);
      break;
    }
...
__global__ void legate_weighted_bincount_1d(const AccessorWO<WT, 1> out,
                                            const AccessorRO<T, 1> in,
                                            const AccessorRO<WT, 1> weights,
                                            const Point<1> origin,
                                            const size_t max,
                                            const T num_bins,
                                            const WT identity)
{
  ...
  WT* bins = (WT*)array;
  // Initialize the bins to 0
  for (int bin = threadIdx.x; bin < num_bins; bin += blockDim.x) bins[bin] = identity;
  __syncthreads();
  // Start reading values and do atomic updates to shared
  size_t offset       = blockIdx.x * blockDim.x + threadIdx.x;
  const size_t stride = gridDim.x * blockDim.x;
  ...
  for (int bin = threadIdx.x; bin < num_bins; bin += blockDim.x) {
    ...
    if (weight != identity)
      SumReduction<WT>::template fold<false /*exclusive*/>(out[bin], bins[bin]);
  }
}

template <typename T, typename WT>
__global__ void legate_weighted_bincount_2d(const AccessorWO<WT, 1> out,
                                            const AccessorRO<T, 2> in,
                                            const AccessorRO<WT, 2> weights,
                                            const Point<2> origin,
                                            const Point<1> pitch,
                                            const size_t max,
                                            const T num_bins,
...
template <typename T, typename WT>
__global__ void legate_weighted_bincount_3d(const AccessorWO<WT, 1> out,
                                            const AccessorRO<T, 3> in,
                                            const AccessorRO<WT, 3> weights,
                                            const Point<3> origin,
                                            const Point<2> pitch,
                                            const size_t max,
                                            const T num_bins,
...
template <typename T, typename WT>
/*static*/ void WeightedBinCountTask<T, WT>::gpu_variant(const Task* task,
                                                         const std::vector<PhysicalRegion>& regions,
                                                         Context ctx,
                                                         Runtime* runtime)
{
  ...
  switch (dim) {
    case 1: {
      ...
      // Launch a kernel with this number of CTAs
      const size_t volume = rect.volume();
      legate_weighted_bincount_1d<T, WT><<<num_ctas, THREADS_PER_BLOCK, bin_size>>>(
        out, in, weights, rect.lo, volume, bin_volume, SumReduction<WT>::identity);
      break;
    }
    case 2: {
      const Rect<2> rect = NumPyProjectionFunctor::unpack_shape<2>(task, derez);
      ...
      const size_t volume = rect.volume();
      const coord_t pitch = rect.hi[1] - rect.lo[1] + 1;
      legate_weighted_bincount_2d<T, WT><<<num_ctas, THREADS_PER_BLOCK, bin_size>>>(
        out, in, weights, rect.lo, pitch, volume, bin_volume, SumReduction<WT>::identity);
      break;
    }
    case 3: {
      const Rect<3> rect = NumPyProjectionFunctor::unpack_shape<3>(task, derez);
      ...
      const coord_t diffz    = rect.hi[2] - rect.lo[2] + 1;
      const coord_t pitch[2] = {diffy * diffz, diffz};
      legate_weighted_bincount_3d<T, WT><<<num_ctas, THREADS_PER_BLOCK, bin_size>>>(
        out, in, weights, rect.lo, Point<2>(pitch), volume, bin_volume, SumReduction<WT>::identity);
      break;
    }
    default: assert(false);
  }
...
#define INSTANTIATE_WEIGHTED_BINCOUNT_VARIANT(task, type, variant)       \
  template void task<type, __half>::variant(                             \
    const Task*, const std::vector<PhysicalRegion>&, Context, Runtime*); \
  template void task<type, float>::variant(                              \
    const Task*, const std::vector<PhysicalRegion>&, Context, Runtime*); \
  template void task<type, double>::variant(                             \
    const Task*, const std::vector<PhysicalRegion>&, Context, Runtime*); \
  template void task<type, int16_t>::variant(                            \
    const Task*, const std::vector<PhysicalRegion>&, Context, Runtime*); \
  template void task<type, int32_t>::variant(                            \
    const Task*, const std::vector<PhysicalRegion>&, Context, Runtime*); \
  template void task<type, int64_t>::variant(                            \
    const Task*, const std::vector<PhysicalRegion>&, Context, Runtime*); \
  template void task<type, uint16_t>::variant(                           \
    const Task*, const std::vector<PhysicalRegion>&, Context, Runtime*); \
  template void task<type, uint32_t>::variant(                           \
    const Task*, const std::vector<PhysicalRegion>&, Context, Runtime*); \
  template void task<type, uint64_t>::variant(                           \
    const Task*, const std::vector<PhysicalRegion>&, Context, Runtime*);
// No bools for now
