...
struct ScanGlobalImplBody<VariantKind::OMP, CODE, DIM> {
		  ...
		  const int prod)
  {
	...
	auto  base = thrust::reduce(thrust::omp::par, &sum_vals[sum_valsp], &sum_vals[sum_valsp] + partition_index[DIM - 1] - 1, thrust::multiplies<VAL>()); // RRRR is the indexing format correct?
