  ...
  cuda_add(RES *B, uint64_t len, uint64_t stride, OP func, RES *block_sum)
{
  ...
  if(remaining_batch == 0){
    remaining_batch = THREADS_PER_BLOCK;
  }
...
template <typename RES, typename OP>
static __global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  batch_scan_cuda_nan(const RES *A, RES *B, uint64_t len, uint64_t stride, OP func, RES identity, RES *block_sum)
{
  __shared__ RES temp[THREADS_PER_BLOCK];
...
template <typename RES, typename OP>
void cuda_scan_nan(const RES *A, RES *B, uint64_t len, uint64_t stride, OP func, RES identity, cudaStream_t stream)
{
  assert(stride != 0);
  uint64_t pad_stride = 1 << (32 - __builtin_clz(stride));
  if(pad_stride > THREADS_PER_BLOCK){
  ...
  batch_scan_cuda_nan<RES, OP><<<grid_dim, THREADS_PER_BLOCK, 0, stream>>>(A, B, len, stride, func, identity, blocked_sum);
  CHECK_CUDA_STREAM(stream);
