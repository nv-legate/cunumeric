...
struct ScanGlobalImplBody<VariantKind::CPU, CODE, DIM> {
		  ...
		  const int prod)
  {
	...
	auto  base = thrust::reduce(thrust::host, &sum_vals[sum_valsp], &sum_vals[sum_valsp] + partition_index[DIM - 1] - 1, thrust::multiplies<VAL>()); // RRRR is the indexing format correct?
