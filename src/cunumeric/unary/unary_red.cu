...
#else
    // cast to 32-bit unsigned to match the argument type of cub match any
    std::uint32_t const same_mask =
      cub::MatchAny<sizeof(unsigned int)>(static_cast<unsigned int>(bucket));
#endif

    std::uint32_t laneid       = cub::LaneId();
