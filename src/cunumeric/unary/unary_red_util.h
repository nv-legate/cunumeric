...
template <UnaryRedCode OP_CODE, legate::Type TYPE_CODE>
struct UnaryRedOp {
  static constexpr bool valid = false;
};

template <legate::Type TYPE_CODE>
struct UnaryRedOp<UnaryRedCode::ALL, TYPE_CODE> {
  static constexpr bool valid = TYPE_CODE != legate::Type::COMPLEX128;
...
struct UnaryRedOp<UnaryRedCode::CONTAINS, TYPE_CODE> {
  ...
  using RHS     = legate::legate_type_of<TYPE_CODE>;
  using VAL     = bool;
  using _RED_OP = UnaryRedOp<UnaryRedCode::SUM, legate::Type::BOOL>;
  using OP      = _RED_OP::OP;
};
