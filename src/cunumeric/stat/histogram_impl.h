...
template<typename element_t,
         template <typename...> typename vect_t>
...
struct lower_bound_op_t{
    lower_bound_op_t(bin_t const* p_bins, size_t n_intervs):
        p_bins_(p_bins),
        n_intervs_(n_intervs)
    {
    }
...
struct transform_op_t{
    transform_op_t(exe_policy_t exe_pol,
                   bin_t const* p_bins,
                   weight_t* p_hist,
                   size_t n_intervs):
        ptr_bins_(p_bins)
                       ...
                       cudaStream_t stream = nullptr)
{
    ...
    //
    thrust::sort_by_key(exe_pol,
                        ptr_src, ptr_src + n_samples,
                        ptr_w);
    ...
    thrust::lower_bound(exe_pol,
                        ptr_src, ptr_src + n_samples,
                        ptr_over, ptr_over + n_intervals + 1,
                        get_raw_ptr(v_offsets),
                        lbop);
    
    segmented_sum_t segsum{exe_pol, ptr_w, ptr_hist, n_intervals, get_raw_ptr(v_offsets), stream, allocator};
