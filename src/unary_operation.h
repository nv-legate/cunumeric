...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM) gpu_unary_op(const Args args, const bool dense) {
...
public:
  ...
  static const int TASK_ID = task_id<UnaryFunction::op_code, NUMPY_NORMAL_VARIANT_OFFSET, result_type, argument_type>;
...
#  pragma omp parallel for schedule(static)
  ...
  static void dispatch_gpu(const Legion::Task* task, const std::vector<Legion::PhysicalRegion>& regions,
    ...
    gpu_unary_op<DIM, UnaryFunction, DeserializedArgs<DIM>><<<blocks, THREADS_PER_BLOCK>>>(args, dense);
  }
