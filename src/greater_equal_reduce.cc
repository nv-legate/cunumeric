...
template<typename T>
/*static*/ bool GreaterEqualReducTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                      Runtime* runtime) {
...
template<typename T>
/*static*/ bool GreaterEqualReducTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                      Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim    = derez.unpack_dimension();
  bool               result = true;
...
INSTANTIATE_ALL_TASKS(GreaterEqualReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_GREATER_EQUAL) * NUMPY_TYPE_OFFSET + NUMPY_REDUCTION_VARIANT_OFFSET)
...
static void __attribute__((constructor)) register_tasks(void) {
  REGISTER_ALL_TASKS_WITH_BOOL_RETURN(legate::numpy::GreaterEqualReducTask)
}
}    // namespace
