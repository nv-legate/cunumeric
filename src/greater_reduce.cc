...
template <typename T>
/*static*/ bool GreaterReducTask<T>::cpu_variant(const Task* task,
...
template <typename T>
/*static*/ bool GreaterReducTask<T>::omp_variant(const Task* task,
                                                 const std::vector<PhysicalRegion>& regions,
                                                 Context ctx,
                                                 Runtime* runtime)
{
...
INSTANTIATE_ALL_TASKS(GreaterReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_GREATER) * NUMPY_TYPE_OFFSET +
