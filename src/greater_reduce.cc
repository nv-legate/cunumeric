...
template<typename T>
/*static*/ bool GreaterReducTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                 Runtime* runtime) {
...
template<typename T>
/*static*/ bool GreaterReducTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                 Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim    = derez.unpack_dimension();
  bool               result = true;
...
INSTANTIATE_ALL_TASKS(GreaterReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_GREATER) * NUMPY_TYPE_OFFSET + NUMPY_REDUCTION_VARIANT_OFFSET)
...
static void __attribute__((constructor)) register_tasks(void) {
  REGISTER_ALL_TASKS_WITH_BOOL_RETURN(legate::numpy::GreaterReducTask)
}
}    // namespace
