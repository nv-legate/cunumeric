...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_vec_norm_2d(const AccessorRW<T, 2> inout,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_vec_norm_3d(const AccessorRW<T, 3> inout,
...
template <typename T>
/*static*/ void NormTask<T>::gpu_variant(const Task* task,
                                         ...
                                         Runtime* runtime)
{
  ...
  switch (init_dim) {
    case 1: {
      ...
      legate_fill_1d<T>
        <<<blocks, THREADS_PER_BLOCK>>>(out, SumReduction<T>::identity, rect.lo, volume);
      break;
    }
    case 2: {
      ...
      legate_fill_2d<T><<<blocks, THREADS_PER_BLOCK>>>(
        out, SumReduction<T>::identity, rect.lo, Point<1>(pitch), volume);
      break;
    }
  ...
  switch (dim) {
    ...
    case 2: {
      ...
      legate_vec_norm_2d<T>
        <<<blocks, threads>>>(inout, in, rect, SumReduction<T>::identity, axis, order);
      break;
    }
    case 3: {
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_vec_norm_reduce_1d(DeferredReduction<SumReduction<T>> result,
  ...
  if (order == 1) {
    ...
    for (unsigned idx = 0; idx < iters; idx++) {
      ...
      if (offset < max) {
        ...
        if (std::is_signed<T>::value && val < T(0)) val = -val;
        T prod = val;
        for (int i = 0; i < (order - 1); i++) ProdReduction<T>::template fold<true>(prod, val);
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_vec_norm_reduce_2d(DeferredReduction<SumReduction<T>> result,
                            const AccessorRO<T, 2> in,
                            const size_t iters,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_vec_norm_reduce_3d(DeferredReduction<SumReduction<T>> result,
                            const AccessorRO<T, 3> in,
                            const size_t iters,
...
template <typename T>
/*static*/ DeferredReduction<SumReduction<T>> NormReducTask<T>::gpu_variant(
  ...
  switch (dim) {
    case 1: {
      ...
      if (blocks >= MAX_REDUCTION_CTAS) {
        ...
        legate_vec_norm_reduce_1d<T><<<MAX_REDUCTION_CTAS, THREADS_PER_BLOCK>>>(
          result, in, iters, rect.lo, volume, order, SumReduction<T>::identity);
      } else {
        legate_vec_norm_reduce_1d<T><<<blocks, THREADS_PER_BLOCK>>>(
          result, in, 1 /*iters*/, rect.lo, volume, order, SumReduction<T>::identity);
      }
      break;
    }
    case 2: {
      ...
      if (blocks >= MAX_REDUCTION_CTAS) {
        const size_t iters = (blocks + MAX_REDUCTION_CTAS - 1) / MAX_REDUCTION_CTAS;
        legate_vec_norm_reduce_2d<T><<<MAX_REDUCTION_CTAS, THREADS_PER_BLOCK>>>(
          result, in, iters, rect.lo, Point<1>(pitch), volume, order, SumReduction<T>::identity);
      } else {
        legate_vec_norm_reduce_2d<T><<<blocks, THREADS_PER_BLOCK>>>(result,
                                                                    in,
                                                                    1 /*iter*/,
                                                                    rect.lo,
                                                                    Point<1>(pitch),
    ...
    case 3: {
      ...
      if (blocks >= MAX_REDUCTION_CTAS) {
        const size_t iters = (blocks + MAX_REDUCTION_CTAS - 1) / MAX_REDUCTION_CTAS;
        legate_vec_norm_reduce_3d<T><<<MAX_REDUCTION_CTAS, THREADS_PER_BLOCK>>>(
          result, in, iters, rect.lo, Point<2>(pitch), volume, order, SumReduction<T>::identity);
      } else {
        legate_vec_norm_reduce_3d<T><<<blocks, THREADS_PER_BLOCK>>>(result,
                                                                    in,
                                                                    1 /*iter*/,
                                                                    rect.lo,
                                                                    Point<2>(pitch),
