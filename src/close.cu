...
__device__ __forceinline__ static double operator*(double lhs, __half rhs)
{
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_close_1d(DeferredValue<bool> result,
                  ...
                  const size_t max)
{
  ...
  int value           = (offset >= max) ? 1
                              : (static_cast<double>(FABS(in1[x] - in2[x])) <=
                                 static_cast<double>(atol + rtol * FABS(in2[x])))
                                  ? 1
                                  : 0;
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_close_2d(DeferredValue<bool> result,
                  ...
                  const size_t max)
{
  ...
  int value           = (offset >= max) ? 1
                              : (static_cast<double>(FABS(in1[x][y] - in2[x][y])) <=
                                 static_cast<double>(atol + rtol * FABS(in2[x][y])))
                                  ? 1
                                  : 0;
  reduce_bool(result, value);
}
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_close_3d(DeferredValue<bool> result,
                  ...
                  const size_t max)
{
  ...
  int value           = (offset >= max) ? 1
                              : (static_cast<double>(FABS(in1[x][y][z] - in2[x][y][z])) <=
                                 static_cast<double>(atol + rtol * FABS(in2[x][y][z])))
                                  ? 1
                                  : 0;
  reduce_bool(result, value);
}
...
template <typename T>
/*static*/ DeferredValue<bool> CloseTask<T>::gpu_variant(const Task* task,
                                                         ...
                                                         Runtime* runtime)
{
  ...
  switch (dim) {
    case 1: {
      ...
      legate_close_1d<T>
        <<<blocks, THREADS_PER_BLOCK>>>(result, in1, in2, rtol, atol, rect.lo, volume);
      break;
    }
    case 2: {
      ...
      legate_close_2d<T><<<blocks, THREADS_PER_BLOCK>>>(
        result, in1, in2, rtol, atol, rect.lo, Point<1>(pitch), volume);
      break;
    }
    case 3: {
      ...
      legate_close_3d<T><<<blocks, THREADS_PER_BLOCK>>>(
        result, in1, in2, rtol, atol, rect.lo, Point<2>(pitch), volume);
      break;
    }
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_close_broadcast_1d(DeferredValue<bool> result,
                            const AccessorRO<T, 1> in1,
                            const T in2,
                            const double rtol,
                            const double atol,
                            ...
                            const size_t max)
{
  ...
  const coord_t x     = origin[0] + offset;
  int value           = (offset >= max) ? 1
                              : FIRST ? ((static_cast<double>(FABS(in2 - in1[x])) <=
                                          static_cast<double>(atol + rtol * FABS(in1[x])))
                                           ? 1
                                           : 0)
                                      : ((static_cast<double>(FABS(in1[x] - in2)) <=
                                          static_cast<double>(atol + rtol * FABS(in2)))
                                           ? 1
                                           : 0);
  reduce_bool(result, value);
}
...
template <typename T, bool FIRST>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_close_broadcast_2d(DeferredValue<bool> result,
                            const AccessorRO<T, 2> in1,
                            const T in2,
                            const double rtol,
                            const double atol,
                            ...
                            const size_t max)
{
  ...
  const coord_t y     = origin[1] + offset % pitch[0];
  int value           = (offset >= max) ? 1
                              : FIRST ? ((static_cast<double>(FABS(in2 - in1[x][y])) <=
                                          static_cast<double>(atol + rtol * FABS(in1[x][y])))
                                           ? 1
                                           : 0)
                                      : ((static_cast<double>(FABS(in1[x][y] - in2)) <=
                                          static_cast<double>(atol + rtol * FABS(in2)))
                                           ? 1
                                           : 0);
  reduce_bool(result, value);
...
template <typename T, bool FIRST>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_close_broadcast_3d(DeferredValue<bool> result,
                            const AccessorRO<T, 3> in1,
                            const T in2,
                            const double rtol,
                            const double atol,
                            ...
                            const size_t max)
{
  ...
  const coord_t z     = origin[2] + (offset % pitch[0]) % pitch[1];
  int value           = (offset >= max) ? 1
                              : FIRST ? ((static_cast<double>(FABS(in2 - in1[x][y][z])) <=
                                          static_cast<double>(atol + rtol * FABS(in1[x][y][z])))
                                           ? 1
                                           : 0)
                                      : ((static_cast<double>(FABS(in1[x][y][z] - in2)) <=
                                          static_cast<double>(atol + rtol * FABS(in2)))
                                           ? 1
                                           : 0);
  reduce_bool(result, value);
...
template <typename T>
/*static*/ DeferredValue<bool> CloseBroadcast<T>::gpu_variant(
  const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime)
{
  ...
  switch (dim) {
    case 1: {
      ...
      const size_t blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
      if (index == 0)
        legate_close_broadcast_1d<T, true>
          <<<blocks, THREADS_PER_BLOCK>>>(result, in1, in2, rtol, atol, rect.lo, volume);
      else
        legate_close_broadcast_1d<T, false>
          <<<blocks, THREADS_PER_BLOCK>>>(result, in1, in2, rtol, atol, rect.lo, volume);
      break;
    }
    case 2: {
      ...
      const coord_t pitch = rect.hi[1] - rect.lo[1] + 1;
      if (index == 0)
        legate_close_broadcast_2d<T, true><<<blocks, THREADS_PER_BLOCK>>>(
          result, in1, in2, rtol, atol, rect.lo, Point<1>(pitch), volume);
      else
        legate_close_broadcast_2d<T, false><<<blocks, THREADS_PER_BLOCK>>>(
          result, in1, in2, rtol, atol, rect.lo, Point<1>(pitch), volume);
      break;
    }
    case 3: {
      ...
      const coord_t pitch[2] = {diffy * diffz, diffz};
      if (index == 0)
        legate_close_broadcast_3d<T, true><<<blocks, THREADS_PER_BLOCK>>>(
          result, in1, in2, rtol, atol, rect.lo, Point<2>(pitch), volume);
      else
        legate_close_broadcast_3d<T, false><<<blocks, THREADS_PER_BLOCK>>>(
          result, in1, in2, rtol, atol, rect.lo, Point<2>(pitch), volume);
      break;
    }
    default: assert(false);
