...
NumPyMapper::NumPyMapper(MapperRuntime* rt, Machine m, TaskID first, TaskID last, ShardingID init)
    : Mapper(rt), machine(m), local_node(get_local_node()), total_nodes(get_total_nodes(m)), mapper_name(create_name(local_node)),
      first_numpy_task_id(first), last_numpy_task_id(last), first_sharding_id(init),
      min_gpu_chunk(extract_env("NUMPY_MIN_GPU_CHUNK", 1 << 20, 2)), min_cpu_chunk(extract_env("NUMPY_MIN_CPU_CHUNK", 1 << 14, 2)),
      min_omp_chunk(extract_env("NUMPY_MIN_OMP_CHUNK", 1 << 17, 2)), eager_fraction(extract_env("NUMPY_EAGER_FRACTION", 16, 1)),
...
//--------------------------------------------------------------------------
{
  ...
  for (std::vector<Processor>::const_iterator it = local_gpus.begin(); it != local_gpus.end(); it++) {
  ...
  for (std::vector<Processor>::const_iterator it = local_omps.begin(); it != local_omps.end(); it++) {
...
NumPyMapper::NumPyMapper(const NumPyMapper& rhs)
    : Mapper(rhs.runtime), machine(rhs.machine), local_node(0), total_nodes(0), mapper_name(NULL), first_numpy_task_id(0),
      last_numpy_task_id(0), first_sharding_id(0), min_gpu_chunk(0), min_cpu_chunk(0), min_omp_chunk(0), eager_fraction(0),
      field_reuse_frac(0), field_reuse_freq(0)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
{
  ...
  if (show_usage != NULL) {
    ...
    for (std::map<FieldMemInfo, InstanceInfos>::const_iterator lit = local_instances.begin(); lit != local_instances.end(); lit++) {
      for (std::vector<InstanceInfo>::const_iterator it = lit->second.instances.begin(); it != lit->second.instances.end(); it++) {
    ...
    const char* memory_kinds[12] = {
        "global", "system", "regmem", "socket",        "zero-copy",     "framebuffer",
        "disk",   "HDF",    "file",   "level 3 cache", "level 2 cache", "level 1 cache",
    };
    for (std::map<Memory, size_t>::const_iterator it = mem_sizes.begin(); it != mem_sizes.end(); it++) {
      assert(it->first.kind() < 12);
      const size_t capacity = it->first.capacity();
      log_numpy.print("Legate.NumPy used %ld bytes of %s memory %llx with "
                      "%ld total bytes (%.2g%%)",
                      it->second, memory_kinds[it->first.kind()], it->first.id, capacity, 100.0 * double(it->second) / capacity);
    }
  }
}
...
//--------------------------------------------------------------------------
void NumPyMapper::select_task_options(const MapperContext ctx, const Task& task, TaskOptions& output)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::premap_task(const MapperContext ctx, const Task& task, const PremapTaskInput& input, PremapTaskOutput& output)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::slice_task(const MapperContext ctx, const Task& task, const SliceTaskInput& input, SliceTaskOutput& output)
//--------------------------------------------------------------------------
{
  ...
  if (task.sharding_space.exists()) sharding_domain = runtime->get_index_space_domain(ctx, task.sharding_space);
  switch (task.target_proc.kind()) {
    case Processor::LOC_PROC: {
      for (Domain::DomainPointIterator itr(input.domain); itr; itr++) {
        const unsigned local_index = functor->localize(itr.p, sharding_domain, total_nodes, local_node) % local_cpus.size();
        output.slices.push_back(TaskSlice(Domain(itr.p, itr.p), local_cpus[local_index], false /*recurse*/, false /*stealable*/));
      }
      break;
    }
    case Processor::TOC_PROC: {
      for (Domain::DomainPointIterator itr(input.domain); itr; itr++) {
        const unsigned local_index = functor->localize(itr.p, sharding_domain, total_nodes, local_node) % local_gpus.size();
        output.slices.push_back(TaskSlice(Domain(itr.p, itr.p), local_gpus[local_index], false /*recurse*/, false /*stealable*/));
      }
      break;
    }
    case Processor::OMP_PROC: {
      for (Domain::DomainPointIterator itr(input.domain); itr; itr++) {
        const unsigned local_index = functor->localize(itr.p, sharding_domain, total_nodes, local_node) % local_omps.size();
        output.slices.push_back(TaskSlice(Domain(itr.p, itr.p), local_omps[local_index], false /*recurse*/, false /*stealable*/));
      }
      break;
    }
    default:
...
//--------------------------------------------------------------------------
{
  ...
  std::map<std::pair<TaskID, Processor::Kind>, VariantID>::const_iterator finder = leaf_variants.find(key);
...
//--------------------------------------------------------------------------
VariantID NumPyMapper::find_variant(const MapperContext ctx, const Task& task, bool subrank, Processor target_proc)
//--------------------------------------------------------------------------
{
  const std::pair<TaskID, Processor::Kind> key(task.task_id, target_proc.kind());
  if (subrank) {
    std::map<std::pair<TaskID, Processor::Kind>, VariantID>::const_iterator finder = inner_variants.find(key);
  ...
  if (result == 0) {
    if (!subrank) {
      ...
      log_numpy.error("Unable to find variant for task %s to run on "
                      "processor %llx. Variant needs op code %d, type code %d, "
                      "and variant code %d. Please request an implementation.",
                      task.get_task_name(), target_proc.id, static_cast<int>(op_code), type_code, variant_code);
    } else
      log_numpy.error("Unable to find subrank variant for task %s "
...
//--------------------------------------------------------------------------
void NumPyMapper::map_task(const MapperContext ctx, const Task& task, const MapTaskInput& input, MapTaskOutput& output)
//--------------------------------------------------------------------------
{
  ...
  for (unsigned idx = 0; idx < task.regions.size(); idx++) {
    ...
    unsigned   index   = 0;
    const bool memoize = ((req.tag & NUMPY_NO_MEMOIZE_TAG) == 0) && (req.privilege != LEGION_REDUCE);
    for (std::set<FieldID>::const_iterator it = req.privilege_fields.begin(); it != req.privilege_fields.end(); it++, index++)
      if (map_numpy_array(ctx, task, idx, req.region, *it, target_memory, task.target_proc, valid, instances[index], memoize,
                          req.redop))
  ...
  while (!needed_acquires.empty() && !runtime->acquire_and_filter_instances(ctx, needed_acquires, true /*filter on acquire*/)) {
    ...
    for (unsigned idx1 = 0; idx1 < task.regions.size(); idx1++) {
      ...
      for (unsigned idx2 = 0; idx2 < instances.size(); idx2++, fit++) {
        ...
        const bool                           memoize = ((req.tag & NUMPY_NO_MEMOIZE_TAG) == 0) && (req.privilege != LEGION_REDUCE);
        if (map_numpy_array(ctx, task, idx1, req.region, fid, target_memory, task.target_proc, valid, instances[idx2], memoize,
                            req.redop))
...
//--------------------------------------------------------------------------
void NumPyMapper::map_replicate_task(const MapperContext ctx, const Task& task, const MapTaskInput& input,
...
//--------------------------------------------------------------------------
bool NumPyMapper::find_existing_instance(LogicalRegion region, FieldID fid, Memory target_memory, PhysicalInstance& result,
...
//--------------------------------------------------------------------------
{
  ...
  for (std::map<Processor, Memory>::const_iterator it = local_frame_buffers.begin(); it != local_frame_buffers.end(); it++) {
  ...
  for (std::map<Processor, Memory>::const_iterator it = local_numa_domains.begin(); it != local_numa_domains.end(); it++) {
...
//--------------------------------------------------------------------------
bool NumPyMapper::map_numpy_array(const MapperContext ctx, const Mappable& mappable, unsigned index, LogicalRegion region,
                                  FieldID fid, Memory target_memory, Processor target_proc,
                                  const std::vector<PhysicalInstance>& valid, PhysicalInstance& result, bool memoize_result,
...
//--------------------------------------------------------------------------
{
  ...
  if (redop != 0) {
    ...
    if (!runtime->create_physical_instance(ctx, target_memory, layout_constraints, regions, result, true /*acquire*/))
  ...
  if (target_proc.exists() && ((target_proc.kind() == Processor::LOC_PROC) || (target_proc.kind() == Processor::OMP_PROC))) {
    Machine::MemoryQuery affinity_mems(machine);
    affinity_mems.has_affinity_to(target_proc);
    for (Machine::MemoryQuery::iterator it = affinity_mems.begin(); it != affinity_mems.end(); it++) {
...
#define LEGATE_DIMFUNC(DN)                                                                                                         \
  case DN: {                                                                                                                       \
    ...
    for (unsigned idx = 0; idx < infos.instances.size(); idx++) {                                                                  \
      ...
      if ((union_volume - (bound_volume + other.volume() - intersect_volume)) > intersect_volume) continue;                        \
    ...
    if (!changed) {                                                                                                                \
      for (std::map<FieldMemInfo, InstanceInfos>::const_iterator it = local_instances.begin(); it != local_instances.end();        \
           it++) {                                                                                                                 \
        if ((it->first.tid != info_key.tid) || (it->first.fid == info_key.fid) || (it->first.memory != info_key.memory)) continue; \
        std::map<LogicalRegion, unsigned>::const_iterator finder = it->second.region_mapping.find(region);                         \
...
#undef LEGATE_DIMFUNC
  ...
  if (overlaps.empty()) {
    ...
    bool   created;
    size_t footprint;
    if (runtime->find_or_create_physical_instance(ctx, target_memory, layout_constraints, regions, result, created,
                                                  true /*acquire*/, memoize_result ? GC_NEVER_PRIORITY : 0, false /*tight bounds*/,
                                                  &footprint)) {
      // We succeeded in making the instance where we want it
      assert(result.exists());
      if (created)
        log_numpy.info("%s created instance %lx containing %zd bytes in memory " IDFMT, mapper_name, result.get_instance_id(),
                       footprint, target_memory.id);
      ...
      if (memoize_result && !result.is_external_instance()) {
        ...
        for (std::set<LogicalRegion>::const_iterator it = other_field_overlaps.begin(); it != other_field_overlaps.end(); it++) {
    ...
    if (info.bounding_box == upper_bound) {
      ...
      bool   created;
      size_t footprint;
      if (runtime->find_or_create_physical_instance(ctx, target_memory, layout_constraints, info.regions, result, created,
                                                    true /*acquire*/, GC_NEVER_PRIORITY, false /*tight bounds*/, &footprint)) {
        // We succeeded in making the instance where we want it
        assert(result.exists());
        if (created)
          log_numpy.info("%s created instance %lx containing %zd bytes in memory " IDFMT, mapper_name, result.get_instance_id(),
    ...
    for (std::vector<unsigned>::const_iterator it = overlaps.begin(); it != overlaps.end(); it++)
      combined_regions.insert(combined_regions.end(), infos.instances[*it].regions.begin(), infos.instances[*it].regions.end());
    // Try to make it
    bool   created;
    size_t footprint;
    if (runtime->find_or_create_physical_instance(ctx, target_memory, layout_constraints, combined_regions, result, created,
                                                  true /*acquire*/, GC_NEVER_PRIORITY, false /*tight bounds*/, &footprint)) {
      // We succeeded in making the instance where we want it
      assert(result.exists());
      if (created)
      ...
      for (std::vector<unsigned>::const_reverse_iterator it = overlaps.crbegin(); it != overlaps.crend(); it++) {
  ...
  if (!valid.empty()) {
    for (std::vector<PhysicalInstance>::const_iterator it = valid.begin(); it != valid.end(); it++) {
...
//--------------------------------------------------------------------------
{
  for (std::vector<PhysicalInstance>::const_iterator it = needed_acquires.begin(); it != needed_acquires.end(); it++) {
    ...
    for (std::map<FieldMemInfo, InstanceInfos>::iterator fit = local_instances.begin(); fit != local_instances.end();
         /*nothing*/) {
...
//--------------------------------------------------------------------------
void NumPyMapper::report_failed_mapping(const Mappable& mappable, unsigned index, Memory target_memory, ReductionOpID redop)
//--------------------------------------------------------------------------
{
  const char* memory_kinds[12] = {
      "global", "system", "regmem", "socket",        "zero-copy",     "framebuffer",
  ...
  switch (mappable.get_mappable_type()) {
    case Mappable::TASK_MAPPABLE: {
      ...
      if (redop > 0)
        log_numpy.error("Mapper %s failed to map reduction (%d) region "
                        "requirement %d of task %s (UID %lld) into %s memory " IDFMT,
                        get_mapper_name(), redop, index, task->get_task_name(), mappable.get_unique_id(),
                        memory_kinds[target_memory.kind()], target_memory.id);
      else
        log_numpy.error("Mapper %s failed to map region requirement %d of "
                        "task %s (UID %lld) into %s memory " IDFMT,
                        get_mapper_name(), index, task->get_task_name(), mappable.get_unique_id(),
                        memory_kinds[target_memory.kind()], target_memory.id);
      break;
    }
    case Mappable::COPY_MAPPABLE: {
      if (redop > 0)
        log_numpy.error("Mapper %s failed to map reduction (%d) region "
                        ...
                        get_mapper_name(), redop, index, mappable.get_unique_id(), memory_kinds[target_memory.kind()],
                        target_memory.id);
      else
        log_numpy.error("Mapper %s failed to map region requirement %d of "
                        "copy (UID %lld) into %s memory " IDFMT,
                        get_mapper_name(), index, mappable.get_unique_id(), memory_kinds[target_memory.kind()], target_memory.id);
      break;
    }
...
//--------------------------------------------------------------------------
void NumPyMapper::select_task_variant(const MapperContext ctx, const Task& task, const SelectVariantInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::postmap_task(const MapperContext ctx, const Task& task, const PostMapInput& input, PostMapOutput& output)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::select_task_sources(const MapperContext ctx, const Task& task, const SelectTaskSrcInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::numpy_select_sources(const MapperContext ctx, const PhysicalInstance& target,
                                       const std::vector<PhysicalInstance>& sources, std::deque<PhysicalInstance>& ranking)
//--------------------------------------------------------------------------
{
  ...
  for (unsigned idx = 0; idx < sources.size(); idx++) {
    ...
    if (finder == source_memories.end()) {
      affinity.clear();
      machine.get_mem_mem_affinity(affinity, location, destination_memory, false /*not just local affinities*/);
  ...
  for (std::vector<std::pair<PhysicalInstance, unsigned>>::const_reverse_iterator it = band_ranking.rbegin();
...
//--------------------------------------------------------------------------
void NumPyMapper::report_profiling(const MapperContext ctx, const Task& task, const TaskProfilingInfo& input)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::select_sharding_functor(const MapperContext ctx, const Task& task, const SelectShardingFunctorInput& input,
...
//--------------------------------------------------------------------------
{
  ...
  if (task.regions.empty()) {
    switch (launch_dim) {
      ...
      case 3:
        return first_sharding_id + NUMPY_SHARD_TILE_3D;
      default:
        LEGATE_ABORT
    }
  ...
  NumPyVariantCode variant_code;
  decode_task_id(task.task_id, op_code, type_code, variant_code);
  if (op_code == NumPyOpCode::NUMPY_DOT) {
    switch (launch_dim) {
      ...
      case 3:    // GEMM
      {
        ...
        switch (key_index) {
          case 0:
            return first_sharding_id + NUMPY_SHARD_TILE_3D_2D_XZ;
          case 1:
            return first_sharding_id + NUMPY_SHARD_TILE_3D_2D_XY;
          case 2:
            return first_sharding_id + NUMPY_SHARD_TILE_3D_2D_YZ;
          default:
            LEGATE_ABORT
        }
      }
  ...
  } else if ((op_code == NumPyOpCode::NUMPY_SUM_RADIX) || (op_code == NumPyOpCode::NUMPY_PROD_RADIX) ||
             (op_code == NumPyOpCode::NUMPY_MIN_RADIX) || (op_code == NumPyOpCode::NUMPY_MAX_RADIX) ||
             (op_code == NumPyOpCode::NUMPY_ARGMIN_RADIX) || (op_code == NumPyOpCode::NUMPY_ARGMAX_RADIX)) {
    switch (launch_dim) {
...
//--------------------------------------------------------------------------
void NumPyMapper::map_inline(const MapperContext ctx, const InlineMapping& inline_op, const MapInlineInput& input,
...
//--------------------------------------------------------------------------
{
  ...
  std::vector<PhysicalInstance> needed_acquires;
  for (std::set<FieldID>::const_iterator it = req.privilege_fields.begin(); it != req.privilege_fields.end(); it++, index++)
    if (map_numpy_array(ctx, inline_op, 0, req.region, *it, local_system_memory, inline_op.parent_task->current_proc, valid,
                        output.chosen_instances[index], false /*memoize*/, req.redop))
  ...
  while (!needed_acquires.empty() && !runtime->acquire_and_filter_instances(ctx, needed_acquires, true /*filter on acquire*/)) {
    ...
    for (unsigned idx = 0; idx < output.chosen_instances.size(); idx++, fit++) {
      ...
      // Now try to remap it
      if (map_numpy_array(ctx, inline_op, 0 /*idx*/, req.region, *fit, local_system_memory, inline_op.parent_task->current_proc,
                          valid, output.chosen_instances[idx], false /*memoize*/))
...
//--------------------------------------------------------------------------
void NumPyMapper::select_inline_sources(const MapperContext ctx, const InlineMapping& inline_op, const SelectInlineSrcInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::report_profiling(const MapperContext ctx, const InlineMapping& inline_op, const InlineProfilingInfo& input)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::map_copy(const MapperContext ctx, const Copy& copy, const MapCopyInput& input, MapCopyOutput& output)
//--------------------------------------------------------------------------
{
  ...
  if (copy.is_index_space) {
    ...
    if (!local_gpus.empty() || !local_omps.empty()) {
      ...
      NumPyShardingFunctor* functor     = find_sharding_functor(sid);
      const unsigned        local_index = functor->localize(copy.index_point, copy.index_domain, total_nodes, local_node);
      if (!local_gpus.empty()) {
  ...
  for (unsigned idx = 0; idx < copy.src_requirements.size(); idx++) {
    ...
    for (std::set<FieldID>::const_iterator it = src_req.privilege_fields.begin(); it != src_req.privilege_fields.end(); it++) {
      if (find_existing_instance(src_req.region, *it, target_memory, output.src_instances[idx][fidx]) ||
          map_numpy_array(ctx, copy, idx, src_req.region, *it, target_memory, Processor::NO_PROC, src_valid,
                          output.src_instances[idx][fidx], memoize_src))
    ...
    fidx                                           = 0;
    const bool memoize_dst = ((dst_req.tag & NUMPY_NO_MEMOIZE_TAG) == 0) && (dst_req.privilege != LEGION_REDUCE);
    for (std::set<FieldID>::const_iterator it = dst_req.privilege_fields.begin(); it != dst_req.privilege_fields.end(); it++) {
      if (((dst_req.redop == 0) && find_existing_instance(dst_req.region, *it, target_memory, output.dst_instances[idx][fidx])) ||
          map_numpy_array(ctx, copy, copy.src_requirements.size() + idx, dst_req.region, *it, target_memory, Processor::NO_PROC,
                          dst_valid, output.dst_instances[idx][fidx], memoize_dst, dst_req.redop))
    ...
    if (idx < copy.src_indirect_requirements.size()) {
      ...
      if (find_existing_instance(src_idx.region, fid, target_memory, output.src_indirect_instances[idx]) ||
          map_numpy_array(ctx, copy, idx, src_idx.region, fid, target_memory, Processor::NO_PROC, idx_valid,
                          output.src_indirect_instances[idx], memoize_idx))
    ...
    if (idx < copy.dst_indirect_requirements.size()) {
      ...
      if (find_existing_instance(dst_idx.region, fid, target_memory, output.dst_indirect_instances[idx]) ||
          map_numpy_array(ctx, copy, idx, dst_idx.region, fid, target_memory, Processor::NO_PROC, idx_valid,
                          output.dst_indirect_instances[idx], memoize_idx))
  ...
  while (!needed_acquires.empty() && !runtime->acquire_and_filter_instances(ctx, needed_acquires, true /*filter on acquire*/)) {
    ...
    for (unsigned idx = 0; idx < copy.src_requirements.size(); idx++) {
      ...
      const bool                           memoize_src = ((src_req.tag & NUMPY_NO_MEMOIZE_TAG) == 0);
      for (std::set<FieldID>::const_iterator it = src_req.privilege_fields.begin(); it != src_req.privilege_fields.end(); it++) {
        if (failed_acquires.find(output.src_instances[idx][fidx]) == failed_acquires.end()) continue;
        if (map_numpy_array(ctx, copy, idx, src_req.region, *it, target_memory, Processor::NO_PROC, src_valid,
                            output.src_instances[idx][fidx], memoize_src))
          needed_acquires.push_back(output.src_instances[idx][fidx]);
      }
      const RegionRequirement& dst_req = copy.dst_requirements[idx];
      ...
      const bool memoize_dst = ((dst_req.tag & NUMPY_NO_MEMOIZE_TAG) == 0) && (dst_req.privilege != LEGION_REDUCE);
      for (std::set<FieldID>::const_iterator it = dst_req.privilege_fields.begin(); it != dst_req.privilege_fields.end(); it++) {
        if (failed_acquires.find(output.dst_instances[idx][fidx]) == failed_acquires.end()) continue;
        if (map_numpy_array(ctx, copy, copy.src_requirements.size() + idx, dst_req.region, *it, target_memory, Processor::NO_PROC,
                            dst_valid, output.dst_instances[idx][fidx], memoize_dst, dst_req.redop))
          needed_acquires.push_back(output.dst_instances[idx][fidx]);
      }
      if (idx < copy.src_indirect_requirements.size()) {
...
//--------------------------------------------------------------------------
void NumPyMapper::select_copy_sources(const MapperContext ctx, const Copy& copy, const SelectCopySrcInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::report_profiling(const MapperContext ctx, const Copy& copy, const CopyProfilingInfo& input)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::select_sharding_functor(const MapperContext ctx, const Copy& copy, const SelectShardingFunctorInput& input,
                                          SelectShardingFunctorOutput& output)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::map_close(const MapperContext ctx, const Close& close, const MapCloseInput& input, MapCloseOutput& output)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::select_close_sources(const MapperContext ctx, const Close& close, const SelectCloseSrcInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::report_profiling(const MapperContext ctx, const Close& close, const CloseProfilingInfo& input)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::select_sharding_functor(const MapperContext ctx, const Close& close, const SelectShardingFunctorInput& input,
                                          SelectShardingFunctorOutput& output)
//--------------------------------------------------------------------------
{
  LEGATE_ABORT
...
//--------------------------------------------------------------------------
void NumPyMapper::map_acquire(const MapperContext ctx, const Acquire& acquire, const MapAcquireInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::speculate(const MapperContext ctx, const Acquire& acquire, SpeculativeOutput& output)
//--------------------------------------------------------------------------
{
  output.speculate = false;
}
...
//--------------------------------------------------------------------------
void NumPyMapper::report_profiling(const MapperContext ctx, const Acquire& acquire, const AcquireProfilingInfo& input)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::select_sharding_functor(const MapperContext ctx, const Acquire& acquire, const SelectShardingFunctorInput& input,
                                          SelectShardingFunctorOutput& output)
//--------------------------------------------------------------------------
{
  LEGATE_ABORT
...
//--------------------------------------------------------------------------
void NumPyMapper::map_release(const MapperContext ctx, const Release& release, const MapReleaseInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::select_release_sources(const MapperContext ctx, const Release& release, const SelectReleaseSrcInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::speculate(const MapperContext ctx, const Release& release, SpeculativeOutput& output)
//--------------------------------------------------------------------------
{
  output.speculate = false;
}
...
//--------------------------------------------------------------------------
void NumPyMapper::report_profiling(const MapperContext ctx, const Release& release, const ReleaseProfilingInfo& input)
//--------------------------------------------------------------------------
{
  // No profiling for legate yet
  LEGATE_ABORT
...
//--------------------------------------------------------------------------
void NumPyMapper::select_sharding_functor(const MapperContext ctx, const Release& release, const SelectShardingFunctorInput& input,
                                          SelectShardingFunctorOutput& output)
//--------------------------------------------------------------------------
{
  LEGATE_ABORT
...
//--------------------------------------------------------------------------
void NumPyMapper::select_partition_projection(const MapperContext ctx, const Partition& partition,
                                              const SelectPartitionProjectionInput& input, SelectPartitionProjectionOutput& output)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::map_partition(const MapperContext ctx, const Partition& partition, const MapPartitionInput& input,
...
//--------------------------------------------------------------------------
{
  ...
  for (std::set<FieldID>::const_iterator it = req.privilege_fields.begin(); it != req.privilege_fields.end(); it++) {
    if (find_existing_instance(req.region, *it, local_system_memory, output.chosen_instances[fidx], Strictness::strict) ||
        map_numpy_array(ctx, partition, 0, req.region, *it, local_system_memory, Processor::NO_PROC, valid,
                        output.chosen_instances[fidx], memoize)) {
  ...
  while (!needed_acquires.empty() && !runtime->acquire_and_filter_instances(ctx, needed_acquires, true /*filter on acquire*/)) {
    ...
    for (unsigned idx = 0; idx < output.chosen_instances.size(); idx++, fit++) {
      if (failed_instances.find(output.chosen_instances[idx]) == failed_instances.end()) continue;
      // Now try to remap it
      if (map_numpy_array(ctx, partition, 0 /*idx*/, req.region, *fit, local_system_memory, Processor::NO_PROC, valid,
                          output.chosen_instances[idx], memoize))
        needed_acquires.push_back(output.chosen_instances[idx]);
    }
  }
}
...
//--------------------------------------------------------------------------
void NumPyMapper::select_partition_sources(const MapperContext ctx, const Partition& partition,
...
//--------------------------------------------------------------------------
void NumPyMapper::report_profiling(const MapperContext ctx, const Partition& partition, const PartitionProfilingInfo& input)
//--------------------------------------------------------------------------
{
  // No profiling yet
  LEGATE_ABORT
...
//--------------------------------------------------------------------------
void NumPyMapper::select_sharding_functor(const MapperContext ctx, const Partition& partition,
...
//--------------------------------------------------------------------------
void NumPyMapper::select_sharding_functor(const MapperContext ctx, const Fill& fill, const SelectShardingFunctorInput& input,
                                          SelectShardingFunctorOutput& output)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::configure_context(const MapperContext ctx, const Task& task, ContextConfigOutput& output)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::select_tunable_value(const MapperContext ctx, const Task& task, const SelectTunableInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::select_sharding_functor(const MapperContext ctx, const MustEpoch& epoch, const SelectShardingFunctorInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::memoize_operation(const MapperContext ctx, const Mappable& mappable, const MemoizeInput& input,
...
//--------------------------------------------------------------------------
void NumPyMapper::map_must_epoch(const MapperContext ctx, const MapMustEpochInput& input, MapMustEpochOutput& output)
//--------------------------------------------------------------------------
{
  // No must epoch launches in legate
  LEGATE_ABORT
...
//--------------------------------------------------------------------------
void NumPyMapper::map_dataflow_graph(const MapperContext ctx, const MapDataflowGraphInput& input, MapDataflowGraphOutput& output)
//--------------------------------------------------------------------------
{
  // Not supported yet
  LEGATE_ABORT
...
//--------------------------------------------------------------------------
void NumPyMapper::select_tasks_to_map(const MapperContext ctx, const SelectMappingInput& input, SelectMappingOutput& output)
//--------------------------------------------------------------------------
{
  // Just map all the ready tasks
  for (std::list<const Task*>::const_iterator it = input.ready_tasks.begin(); it != input.ready_tasks.end(); it++)
    output.map_tasks.insert(*it);
}

//--------------------------------------------------------------------------
void NumPyMapper::select_steal_targets(const MapperContext ctx, const SelectStealingInput& input, SelectStealingOutput& output)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::permit_steal_request(const MapperContext ctx, const StealRequestInput& input, StealRequestOutput& output)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
void NumPyMapper::decode_task_id(TaskID tid, NumPyOpCode& op_code, LegateTypeCode& type_code, NumPyVariantCode& variant_code)
//--------------------------------------------------------------------------
{
...
//--------------------------------------------------------------------------
/*static*/ unsigned NumPyMapper::extract_env(const char* env_name, const unsigned default_value, const unsigned test_value)
//--------------------------------------------------------------------------
{
