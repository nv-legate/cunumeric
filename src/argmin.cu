...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_argmin_2d(const AccessorRW<Argval<T>, 2> inout,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_argmin_3d(const AccessorRW<Argval<T>, 3> inout,
...
template <typename T>
/*static*/ void ArgminTask<T>::gpu_variant(const Task* task,
                                           ...
                                           Runtime* runtime)
{
  ...
  switch (init_dim) {
    case 1: {
      ...
      legate_fill_1d<Argval<T>>
        <<<blocks, THREADS_PER_BLOCK>>>(out, ArgminReduction<T>::identity, rect.lo, volume);
      break;
    }
    case 2: {
      ...
      legate_fill_2d<Argval<T>><<<blocks, THREADS_PER_BLOCK>>>(
        out, ArgminReduction<T>::identity, rect.lo, Point<1>(pitch), volume);
      break;
    }
...
template <typename T>
/*static*/ DeferredReduction<ArgminReduction<T>> ArgminReducTask<T>::gpu_variant(
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_argmin_radix_1d(const AccessorWO<Argval<T>, 1> out,
                         const ArgminRadixArgs<T, 1> args,
                         const size_t argmax,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_argmin_radix_2d(const AccessorWO<Argval<T>, 2> out,
                         const ArgminRadixArgs<T, 2> args,
                         const size_t argmax,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_argmin_radix_3d(const AccessorWO<Argval<T>, 3> out,
                         const ArgminRadixArgs<T, 3> args,
                         const size_t argmax,
...
template <typename T>
/*static*/ void ArgminRadixTask<T>::gpu_variant(const Task* task,
                                                const std::vector<PhysicalRegion>& regions,
                                                Context ctx,
                                                Runtime* runtime)
{
  ...
  switch (dim) {
    case 1: {
      ...
      for (unsigned idx = 1; idx < task->regions.size(); idx++)
        if (task->regions[idx].region.exists())
          args.in[num_inputs++] = derez.unpack_accessor_RO<Argval<T>, 1>(
            regions[idx], rect, extra_dim_in, offset + idx - 1);
      const size_t volume = rect.volume();
      const size_t blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
      legate_argmin_radix_1d<T>
    ...
    case 2: {
      ...
      for (unsigned idx = 1; idx < task->regions.size(); idx++)
        if (task->regions[idx].region.exists())
          args.in[num_inputs++] = derez.unpack_accessor_RO<Argval<T>, 2>(
            regions[idx], rect, extra_dim_in, offset + idx - 1);
      const size_t volume = rect.volume();
      const size_t blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
      const coord_t pitch = rect.hi[1] - rect.lo[1] + 1;
      legate_argmin_radix_2d<T>
    ...
    case 3: {
      ...
      unsigned num_inputs = 0;
      for (unsigned idx = 1; idx < task->regions.size(); idx++)
        args.in[num_inputs++] = derez.unpack_accessor_RO<Argval<T>, 3>(
          regions[idx], rect, extra_dim_in, offset + idx - 1);
      const size_t volume    = rect.volume();
      const size_t blocks    = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
