...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_max_2d(const AccessorRW<T, 2> inout,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_max_3d(const AccessorRW<T, 3> inout,
...
template <typename T>
/*static*/ void MaxTask<T>::gpu_variant(const Task* task,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_max_reduce_1d(DeferredReduction<MaxReduction<T>> result,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_max_reduce_2d(DeferredReduction<MaxReduction<T>> result,
                       const AccessorRO<T, 2> in,
                       const size_t iters,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_max_reduce_3d(DeferredReduction<MaxReduction<T>> result,
                       const AccessorRO<T, 3> in,
                       const size_t iters,
...
template <typename T>
/*static*/ DeferredReduction<MaxReduction<T>> MaxReducTask<T>::gpu_variant(
  ...
  switch (dim) {
    case 1: {
      ...
      if (blocks >= MAX_REDUCTION_CTAS) {
        ...
        legate_max_reduce_1d<T><<<MAX_REDUCTION_CTAS, THREADS_PER_BLOCK>>>(
          result, in, iters, rect.lo, volume, MaxReduction<T>::identity);
      } else {
        legate_max_reduce_1d<T><<<blocks, THREADS_PER_BLOCK>>>(
          result, in, 1 /*iter*/, rect.lo, volume, MaxReduction<T>::identity);
      }
      break;
    }
    case 2: {
      ...
      if (blocks >= MAX_REDUCTION_CTAS) {
        const size_t iters = (blocks + MAX_REDUCTION_CTAS - 1) / MAX_REDUCTION_CTAS;
        legate_max_reduce_2d<T><<<MAX_REDUCTION_CTAS, THREADS_PER_BLOCK>>>(
          result, in, iters, rect.lo, Point<1>(pitch), volume, MaxReduction<T>::identity);
      } else {
        legate_max_reduce_2d<T><<<blocks, THREADS_PER_BLOCK>>>(
          result, in, 1 /*iter*/, rect.lo, Point<1>(pitch), volume, MaxReduction<T>::identity);
      }
      break;
    }
    case 3: {
      ...
      if (blocks >= MAX_REDUCTION_CTAS) {
        const size_t iters = (blocks + MAX_REDUCTION_CTAS - 1) / MAX_REDUCTION_CTAS;
        legate_max_reduce_3d<T><<<MAX_REDUCTION_CTAS, THREADS_PER_BLOCK>>>(
          result, in, iters, rect.lo, Point<2>(pitch), volume, MaxReduction<T>::identity);
      } else {
        legate_max_reduce_3d<T><<<blocks, THREADS_PER_BLOCK>>>(
          result, in, 1 /*iter*/, rect.lo, Point<2>(pitch), volume, MaxReduction<T>::identity);
      }
      break;
    }
    default: assert(false);
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_1d(const AccessorWO<T, 1> out,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM) legate_binmax_1d_inplace(
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_2d(const AccessorWO<T, 2> out,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_2d_inplace(const AccessorRW<T, 2> out,
                           const AccessorRO<T, 2> in,
                           const Point<2> origin,
                           const Point<1> pitch,
                           const size_t max)
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_3d(const AccessorWO<T, 3> out,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_3d_inplace(const AccessorRW<T, 3> out,
                           const AccessorRO<T, 3> in,
                           const Point<3> origin,
                           const Point<2> pitch,
                           const size_t max)
...
template <typename T>
/*static*/ void BinMaxTask<T>::gpu_variant(const Task* task,
                                           const std::vector<PhysicalRegion>& regions,
                                           Context ctx,
                                           Runtime* runtime)
{
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_broadcast_1d(const AccessorWO<T, 1> out,
                             const AccessorRO<T, 1> in1,
                             const T in2,
                             const Point<1> origin,
                             const size_t max)
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_broadcast_1d_inplace(const AccessorRW<T, 1> out,
                                     const T in,
                                     const Point<1> origin,
                                     const size_t max)
{
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_broadcast_2d(const AccessorWO<T, 2> out,
                             const AccessorRO<T, 2> in1,
                             const T in2,
                             const Point<2> origin,
                             const Point<1> pitch,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_broadcast_2d_inplace(const AccessorRW<T, 2> out,
                                     const T in,
                                     const Point<2> origin,
                                     const Point<1> pitch,
                                     const size_t max)
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_broadcast_3d(const AccessorWO<T, 3> out,
                             const AccessorRO<T, 3> in1,
                             const T in2,
                             const Point<3> origin,
                             const Point<2> pitch,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_binmax_broadcast_3d_inplace(const AccessorRW<T, 3> out,
                                     const T in,
                                     const Point<3> origin,
                                     const Point<2> pitch,
                                     const size_t max)
...
template <typename T>
/*static*/ void BinMaxBroadcast<T>::gpu_variant(const Task* task,
                                                const std::vector<PhysicalRegion>& regions,
                                                Context ctx,
                                                Runtime* runtime)
{
  ...
  switch (dim) {
    case 1: {
      ...
      if (task->regions.size() == 1) {
        ...
        const size_t volume        = rect.volume();
        const size_t blocks        = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        legate_binmax_broadcast_1d_inplace<T>
    ...
    case 2: {
      ...
      if (task->regions.size() == 1) {
        ...
        const size_t blocks        = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        const coord_t pitch        = rect.hi[1] - rect.lo[1] + 1;
        legate_binmax_broadcast_2d_inplace<T>
    ...
    case 3: {
      ...
      if (task->regions.size() == 1) {
        ...
        const coord_t diffz        = rect.hi[2] - rect.lo[2] + 1;
        const coord_t pitch[2]     = {diffy * diffz, diffz};
        legate_binmax_broadcast_3d_inplace<T>
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_max_radix_1d(const AccessorWO<T, 1> out,
                      const MaxRadixArgs<T, 1> args,
                      const size_t argmax,
                      const Point<1> origin,
                      const size_t max)
{
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_max_radix_2d(const AccessorWO<T, 2> out,
                      const MaxRadixArgs<T, 2> args,
                      const size_t argmax,
                      const Point<2> origin,
                      const Point<1> pitch,
                      const size_t max)
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_max_radix_3d(const AccessorWO<T, 3> out,
                      const MaxRadixArgs<T, 3> args,
                      const size_t argmax,
                      const Point<3> origin,
                      const Point<2> pitch,
                      const size_t max)
...
template <typename T>
/*static*/ void MaxRadixTask<T>::gpu_variant(const Task* task,
                                             const std::vector<PhysicalRegion>& regions,
                                             Context ctx,
                                             Runtime* runtime)
{
