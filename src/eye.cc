...
template<typename T>
/*static*/ void EyeTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                        Runtime* runtime) {
  ...
  for (coord_t idx = 0; idx < distance; idx++)
    out[start[0] + idx][start[1] + idx] = T{1};
}

#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ void EyeTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                        Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  // We know this is 2-D
  const Rect<2>          rect = NumPyProjectionFunctor::unpack_shape<2>(task, derez);
...
#  pragma omp parallel for
  for (coord_t idx = 0; idx < distance; idx++)
    out[start[0] + idx][start[1] + idx] = 1;
}
#endif

INSTANTIATE_ALL_TASKS(EyeTask, static_cast<int>(NumPyOpCode::NUMPY_EYE) * NUMPY_TYPE_OFFSET + NUMPY_NORMAL_VARIANT_OFFSET)
...
namespace    // unnamed
{
static void __attribute__((constructor)) register_tasks(void) { REGISTER_ALL_TASKS(legate::numpy::EyeTask) }
}    // namespace
