...
template <typename T>
/*static*/ void TileTask<T>::cpu_variant(const Task* task,
                                         ...
                                         Runtime* runtime)
{
  ...
  switch (dst_dim) {
    case 1: {
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] = in[x % bounds[0]];
      break;
    }
    case 2: {
      ...
      switch (src_dim) {
        case 1: {
          ...
          for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
            for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++) out[x][y] = in[y % bounds[0]];
          break;
        }
        case 2: {
    ...
    case 3: {
      ...
      switch (src_dim) {
        case 1: {
          ...
          for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
            for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
              for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++) out[x][y][z] = in[z % bounds[0]];
          break;
        }
        case 2: {
          const Point<2> bounds     = derez.unpack_point<2>();
...
template <typename T>
/*static*/ void TileTask<T>::omp_variant(const Task* task,
                                         const std::vector<PhysicalRegion>& regions,
                                         Context ctx,
                                         Runtime* runtime)
{
...
INSTANTIATE_ALL_TASKS(TileTask,
                      static_cast<int>(NumPyOpCode::NUMPY_TILE) * NUMPY_TYPE_OFFSET +
...
{
  REGISTER_ALL_TASKS(legate::numpy::TileTask)
}
}  // namespace
