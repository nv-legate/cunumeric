...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_uniform_rand_1d(const AccessorWO<double, 1> out,
                         const Point<1> origin,
                         const Point<1> strides,
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_uniform_rand_2d(const AccessorWO<double, 2> out,
                         const Point<2> origin,
                         const Point<1> pitch,
                         const Point<2> strides,
                         const unsigned epoch,
                         const size_t max)
{
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_uniform_rand_3d(const AccessorWO<double, 3> out,
                         const Point<3> origin,
                         const Point<2> pitch,
                         const Point<3> strides,
                         const unsigned epoch,
                         const size_t max)
{
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
...
/*static*/ void RandUniformTask::gpu_variant(const Task* task,
                                             ...
                                             Runtime* runtime)
{
  ...
  switch (dim) {
    ...
    case 2: {
      ...
      legate_uniform_rand_2d<<<blocks, THREADS_PER_BLOCK>>>(
        out, rect.lo, pitch, strides, epoch, volume);
      break;
    }
    case 3: {
      ...
      legate_uniform_rand_3d<<<blocks, THREADS_PER_BLOCK>>>(
        out, rect.lo, Point<2>(pitch), strides, epoch, volume);
      break;
    }
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_normal_rand_1d(const AccessorWO<double, 1> out,
                        const Point<1> origin,
                        const Point<1> strides,
                        const unsigned epoch,
                        const size_t max)
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_normal_rand_2d(const AccessorWO<double, 2> out,
                        const Point<2> origin,
                        const Point<1> pitch,
                        const Point<2> strides,
                        const unsigned epoch,
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_normal_rand_3d(const AccessorWO<double, 3> out,
                        const Point<3> origin,
                        const Point<2> pitch,
                        const Point<3> strides,
                        const unsigned epoch,
...
/*static*/ void RandNormalTask::gpu_variant(const Task* task,
                                            const std::vector<PhysicalRegion>& regions,
                                            Context ctx,
                                            Runtime* runtime)
{
  ...
  switch (dim) {
    ...
    case 2: {
      ...
      const size_t blocks             = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
      const coord_t pitch             = rect.hi[1] - rect.lo[1] + 1;
      legate_normal_rand_2d<<<blocks, THREADS_PER_BLOCK>>>(
        out, rect.lo, pitch, strides, epoch, volume);
      break;
    }
    case 3: {
      ...
      const coord_t diffz             = rect.hi[2] - rect.lo[2] + 1;
      const coord_t pitch[2]          = {diffy * diffz, diffz};
      legate_normal_rand_3d<<<blocks, THREADS_PER_BLOCK>>>(
        out, rect.lo, Point<2>(pitch), strides, epoch, volume);
      break;
    }
    default: assert(false);
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_integer_rand_1d(const AccessorWO<T, 1> out,
                         const Point<1> origin,
                         const Point<1> strides,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_integer_rand_2d(const AccessorWO<T, 2> out,
                         const Point<2> origin,
                         const Point<1> pitch,
                         const Point<2> strides,
                         const unsigned long long low,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_integer_rand_3d(const AccessorWO<T, 3> out,
                         const Point<3> origin,
                         const Point<2> pitch,
                         const Point<3> strides,
                         const unsigned long long low,
...
template <typename T>
/*static*/ void RandIntegerTask<T>::gpu_variant(const Task* task,
                                                const std::vector<PhysicalRegion>& regions,
                                                Context ctx,
                                                Runtime* runtime)
{
  ...
  switch (dim) {
    case 1: {
      ...
      const size_t volume        = rect.volume();
      const size_t blocks        = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
      legate_integer_rand_1d<T>
        <<<blocks, THREADS_PER_BLOCK>>>(out, rect.lo, strides, low, diff, epoch, volume);
      break;
    }
    case 2: {
      const Rect<2> rect = NumPyProjectionFunctor::unpack_shape<2>(task, derez);
      ...
      const size_t blocks        = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
      const coord_t pitch        = rect.hi[1] - rect.lo[1] + 1;
      legate_integer_rand_2d<T>
        <<<blocks, THREADS_PER_BLOCK>>>(out, rect.lo, pitch, strides, low, diff, epoch, volume);
      break;
    }
    case 3: {
      const Rect<3> rect = NumPyProjectionFunctor::unpack_shape<3>(task, derez);
      ...
      const coord_t diffz        = rect.hi[2] - rect.lo[2] + 1;
      const coord_t pitch[2]     = {diffy * diffz, diffz};
      legate_integer_rand_3d<T><<<blocks, THREADS_PER_BLOCK>>>(
        out, rect.lo, Point<2>(pitch), strides, low, diff, epoch, volume);
      break;
    }
    default: assert(false);
  }
