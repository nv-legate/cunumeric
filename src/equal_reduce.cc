...
template <typename T>
/*static*/ bool EqualReducTask<T>::cpu_variant(const Task* task,
...
template <typename T>
/*static*/ bool EqualReducTask<T>::omp_variant(const Task* task,
                                               const std::vector<PhysicalRegion>& regions,
                                               Context ctx,
                                               Runtime* runtime)
{
...
INSTANTIATE_ALL_TASKS(EqualReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_EQUAL) * NUMPY_TYPE_OFFSET +
...
{
  REGISTER_ALL_TASKS_WITH_BOOL_RETURN(legate::numpy::EqualReducTask)
}
}  // namespace
