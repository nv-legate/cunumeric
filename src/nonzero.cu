...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_nonzero_2d(const AccessorRW<uint64_t, 2> inout,
                    ...
                    const int axis)
{
  ...
  if (value != identity)
    SumReduction<uint64_t>::template fold<false /*exclusive*/>(inout[p], value);
}

template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_nonzero_3d(const AccessorRW<uint64_t, 3> inout,
...
template <typename T>
/*static*/ void CountNonzeroTask<T>::gpu_variant(const Task* task,
                                                 ...
                                                 Runtime* runtime)
{
  ...
  switch (init_dim) {
    case 1: {
      ...
      legate_fill_1d<uint64_t>
        <<<blocks, THREADS_PER_BLOCK>>>(out, SumReduction<uint64_t>::identity, rect.lo, volume);
      break;
    }
    case 2: {
      ...
      legate_fill_2d<uint64_t><<<blocks, THREADS_PER_BLOCK>>>(
        out, SumReduction<uint64_t>::identity, rect.lo, Point<1>(pitch), volume);
      break;
    }
  ...
  switch (dim) {
    ...
    case 2: {
      ...
      legate_nonzero_2d<T>
        <<<blocks, threads>>>(inout, in, rect, SumReduction<uint64_t>::identity, axis);
      break;
    }
    case 3: {
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_nonzero_reduce_1d(const DeferredBuffer<uint64_t, 1> buffer,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_nonzero_reduce_2d(const DeferredBuffer<uint64_t, 1> buffer,
                           const AccessorRO<T, 2> in,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_nonzero_reduce_3d(const DeferredBuffer<uint64_t, 1> buffer,
                           const AccessorRO<T, 3> in,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_buffer_nonzero_reduce(const DeferredBuffer<T, 1> in,
                               const DeferredBuffer<uint64_t, 1> out,
                               const size_t max,
                               const uint64_t identity)
{
  uint64_t value      = identity;
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_final_nonzero_reduce(const DeferredBuffer<T, 1> in,
                              const DeferredReduction<SumReduction<uint64_t>> out,
                              const size_t max,
                              const uint64_t identity)
{
  uint64_t value      = identity;
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM) legate_final_nonzero_reduce(
  const DeferredBuffer<T, 1> in, T& out, const size_t max, const uint64_t identity)
{
  uint64_t value      = identity;
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset < max) value += in.read(offset);
...
namespace detail {
template <typename T>
/*static*/ std::tuple<coord_t, DeferredBuffer<uint64_t, 1>> count_nonzero_reduc_task_gpu_helper(
  const Task* task, const std::vector<PhysicalRegion>& regions, LegateDeserializer& derez)
{
  const int dim = derez.unpack_dimension();
  ...
  switch (dim) {
    case 1: {
      ...
      legate_nonzero_reduce_1d<T><<<blocks, THREADS_PER_BLOCK>>>(
        bufferA, in, rect.lo, volume, SumReduction<uint64_t>::identity);
      volume = blocks;
      break;
    }
    case 2: {
      ...
      bufferA             = DeferredBuffer<uint64_t, 1>(Memory::GPU_FB_MEM, Domain(bounds));
      const coord_t pitch = rect.hi[1] - rect.lo[1] + 1;
      legate_nonzero_reduce_2d<T><<<blocks, THREADS_PER_BLOCK>>>(
        bufferA, in, rect.lo, Point<1>(pitch), volume, SumReduction<uint64_t>::identity);
      volume = blocks;
      break;
    }
    case 3: {
      ...
      legate_nonzero_reduce_3d<T><<<blocks, THREADS_PER_BLOCK>>>(
        bufferA, in, rect.lo, Point<2>(pitch), volume, SumReduction<uint64_t>::identity);
      volume = blocks;
      break;
    }
    default: assert(false);
  ...
  if (volume > THREADS_PER_BLOCK) {
    ...
    while (volume > THREADS_PER_BLOCK) {
      ...
      if (forward) {
        legate_buffer_nonzero_reduce<uint64_t><<<blocks, THREADS_PER_BLOCK>>>(
          bufferA, bufferB, volume, SumReduction<uint64_t>::identity);
        forward = false;
      } else {
        legate_buffer_nonzero_reduce<uint64_t><<<blocks, THREADS_PER_BLOCK>>>(
          bufferB, bufferA, volume, SumReduction<uint64_t>::identity);
        forward = true;
      }
      volume = blocks;
    }
...
template <typename T>
/*static*/ DeferredReduction<SumReduction<uint64_t>> CountNonzeroReducTask<T>::gpu_variant(
  const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime)
{
  ...
  if (volume > 0)
    legate_final_nonzero_reduce<uint64_t>
      <<<1, THREADS_PER_BLOCK>>>(buffer, result, volume, SumReduction<uint64_t>::identity);
  return result;
}

template <typename T>
/*static*/ void CountNonzeroReducWriteTask<T>::gpu_variant(
  const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime)
{
  LegateDeserializer derez(task->args, task->arglen);
  coord_t volume{};
...
INSTANTIATE_DEFERRED_REDUCTION_ARG_RETURN_TASK_VARIANT(CountNonzeroReducTask,
                                                       SumReduction,
                                                       gpu_variant)
