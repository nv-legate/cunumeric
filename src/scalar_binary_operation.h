...
class ScalarBinaryOperationTask : public NumPyTask<Derived> {
 ...
 public:
  static_assert(std::is_same<first_argument_type, second_argument_type>::value,
                "ScalarBinaryOperationTask currently requires first_argument_type and "
                "second_argument_type to be the same type.");
  static constexpr int TASK_ID = task_id<BinaryFunction::op_code,
                                 ...
                                 Legion::Runtime* runtime)
  {
    ...
    first_argument_type lhs =
      task->futures[0].get_result<first_argument_type>(true /*silence warnings*/);
    second_argument_type rhs =
      task->futures[1].get_result<second_argument_type>(true /*silence warnings*/);
 ...
 private:
  struct StaticRegistrar {
    StaticRegistrar()
    {
      ScalarBinaryOperationTask::template register_variants_with_return<result_type,
                                                                        first_argument_type>();
    }
  };
