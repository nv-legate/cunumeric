...
template <typename T>
/*static*/ void IntModTask<T>::cpu_variant(const Task* task,
  ...
  switch (dim) {
    case 1: {
      ...
      if (task->regions.size() == 2) {
        ...
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] %= in[x];
      } else {
        ...
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] = in1[x] % in2[x];
      }
      break;
    }
    case 2: {
      ...
      if (task->regions.size() == 2) {
        ...
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++) out[x][y] %= in[x][y];
      } else {
        ...
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++) out[x][y] = in1[x][y] % in2[x][y];
      }
      break;
    }
    case 3: {
      ...
      if (task->regions.size() == 2) {
        ...
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
            for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++) out[x][y][z] %= in[x][y][z];
      } else {
...
template <typename T>
/*static*/ void IntModTask<T>::omp_variant(const Task* task,
                                           const std::vector<PhysicalRegion>& regions,
                                           Context ctx,
                                           Runtime* runtime)
{
...
template <typename T>
/*static*/ void RealModTask<T>::cpu_variant(const Task* task,
                                            const std::vector<PhysicalRegion>& regions,
                                            Context ctx,
                                            Runtime* runtime)
{
  ...
  switch (dim) {
    case 1: {
      ...
      if (task->regions.size() == 2) {
        const AccessorRW<T, 1> out = derez.unpack_accessor_RW<T, 1>(regions[0], rect);
        const AccessorRO<T, 1> in  = derez.unpack_accessor_RO<T, 1>(regions[1], rect);
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] = python_mod(out[x], in[x]);
      } else {
        const AccessorWO<T, 1> out = derez.unpack_accessor_WO<T, 1>(regions[0], rect);
        const AccessorRO<T, 1> in1 = derez.unpack_accessor_RO<T, 1>(regions[1], rect);
        const AccessorRO<T, 1> in2 = derez.unpack_accessor_RO<T, 1>(regions[2], rect);
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] = python_mod(in1[x], in2[x]);
      }
      break;
    }
    case 2: {
...
#ifdef LEGATE_USE_OPENMP
template <typename T>
/*static*/ void RealModTask<T>::omp_variant(const Task* task,
                                            const std::vector<PhysicalRegion>& regions,
                                            Context ctx,
                                            Runtime* runtime)
{
...
template <typename T>
/*static*/ void IntModBroadcast<T>::cpu_variant(const Task* task,
                                                const std::vector<PhysicalRegion>& regions,
                                                Context ctx,
                                                Runtime* runtime)
{
  ...
  switch (dim) {
    case 1: {
      ...
      if (task->regions.size() == 1) {
        const AccessorRW<T, 1> out = derez.unpack_accessor_RW<T, 1>(regions[0], rect);
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] %= in2;
      } else {
        const AccessorWO<T, 1> out = derez.unpack_accessor_WO<T, 1>(regions[0], rect);
        const AccessorRO<T, 1> in1 = derez.unpack_accessor_RO<T, 1>(regions[1], rect);
        ...
        if (index == 0) {
          for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] = in2 % in1[x];
        } else {
          for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] = in1[x] % in2;
        }
      }
      break;
    }
    case 2: {
      ...
      if (task->regions.size() == 1) {
        const AccessorRW<T, 2> out = derez.unpack_accessor_RW<T, 2>(regions[0], rect);
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++) out[x][y] %= in2;
      } else {
        const AccessorWO<T, 2> out = derez.unpack_accessor_WO<T, 2>(regions[0], rect);
        const AccessorRO<T, 2> in1 = derez.unpack_accessor_RO<T, 2>(regions[1], rect);
        const unsigned index       = derez.unpack_32bit_uint();
        ...
        if (index == 0) {
          for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
            for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++) out[x][y] = in2 % in1[x][y];
        } else {
          for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
            for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++) out[x][y] = in1[x][y] % in2;
        }
      }
      break;
    }
    case 3: {
      ...
      if (task->regions.size() == 1) {
        ...
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
            for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++) out[x][y][z] %= in2;
      } else {
        const AccessorWO<T, 3> out = derez.unpack_accessor_WO<T, 3>(regions[0], rect);
        const AccessorRO<T, 3> in1 = derez.unpack_accessor_RO<T, 3>(regions[1], rect);
        const unsigned index       = derez.unpack_32bit_uint();
        ...
        if (index == 0) {
          for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
            for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
              for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++) out[x][y][z] = in2 % in1[x][y][z];
        } else {
          for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
            for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
              for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++) out[x][y][z] = in1[x][y][z] % in2;
        }
      }
      break;
    }
...
#ifdef LEGATE_USE_OPENMP
template <typename T>
/*static*/ void IntModBroadcast<T>::omp_variant(const Task* task,
                                                const std::vector<PhysicalRegion>& regions,
                                                Context ctx,
                                                Runtime* runtime)
{
...
template <typename T>
/*static*/ void RealModBroadcast<T>::cpu_variant(const Task* task,
                                                 const std::vector<PhysicalRegion>& regions,
                                                 Context ctx,
                                                 Runtime* runtime)
{
  ...
  switch (dim) {
    case 1: {
      ...
      if (task->regions.size() == 1) {
        const AccessorRW<T, 1> out = derez.unpack_accessor_RW<T, 1>(regions[0], rect);
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] = python_mod(out[x], in2);
      } else {
        const AccessorWO<T, 1> out = derez.unpack_accessor_WO<T, 1>(regions[0], rect);
        const AccessorRO<T, 1> in1 = derez.unpack_accessor_RO<T, 1>(regions[1], rect);
        const unsigned index       = derez.unpack_32bit_uint();
        assert((index == 0) || (index == 1));
        if (index == 0) {
          for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] = python_mod(in2, in1[x]);
        } else {
          for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++) out[x] = python_mod(in1[x], in2);
        }
      }
      break;
    }
    case 2: {
      ...
      if (task->regions.size() == 1) {
        const AccessorRW<T, 2> out = derez.unpack_accessor_RW<T, 2>(regions[0], rect);
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++) out[x][y] = python_mod(out[x][y], in2);
      } else {
        const AccessorWO<T, 2> out = derez.unpack_accessor_WO<T, 2>(regions[0], rect);
        const AccessorRO<T, 2> in1 = derez.unpack_accessor_RO<T, 2>(regions[1], rect);
        const unsigned index       = derez.unpack_32bit_uint();
...
#ifdef LEGATE_USE_OPENMP
template <typename T>
/*static*/ void RealModBroadcast<T>::omp_variant(const Task* task,
                                                 const std::vector<PhysicalRegion>& regions,
                                                 Context ctx,
                                                 Runtime* runtime)
{
...
template <typename T>
/*static*/ T IntModScalar<T>::cpu_variant(const Task* task,
                                          const std::vector<PhysicalRegion>& regions,
                                          Context ctx,
                                          Runtime* runtime)
{
...
template <typename T>
/*static*/ T RealModScalar<T>::cpu_variant(const Task* task,
                                           const std::vector<PhysicalRegion>& regions,
                                           Context ctx,
                                           Runtime* runtime)
{
...
INSTANTIATE_INT_TASKS(IntModTask,
                      static_cast<int>(NumPyOpCode::NUMPY_MOD) * NUMPY_TYPE_OFFSET +
                        NUMPY_NORMAL_VARIANT_OFFSET)
INSTANTIATE_REAL_TASKS(RealModTask,
                       static_cast<int>(NumPyOpCode::NUMPY_MOD) * NUMPY_TYPE_OFFSET +
                         NUMPY_NORMAL_VARIANT_OFFSET)
...
INSTANTIATE_REAL_TASKS(RealModBroadcast,
                       static_cast<int>(NumPyOpCode::NUMPY_MOD) * NUMPY_TYPE_OFFSET +
                         NUMPY_BROADCAST_VARIANT_OFFSET)
INSTANTIATE_INT_TASKS(IntModScalar,
                      static_cast<int>(NumPyOpCode::NUMPY_MOD) * NUMPY_TYPE_OFFSET +
                        NUMPY_SCALAR_VARIANT_OFFSET)
INSTANTIATE_REAL_TASKS(RealModScalar,
                       static_cast<int>(NumPyOpCode::NUMPY_MOD) * NUMPY_TYPE_OFFSET +
                         NUMPY_SCALAR_VARIANT_OFFSET)
