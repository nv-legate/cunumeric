...
/*static*/ void DotTask<T>::set_layout_constraints(LegateVariant variant,
...
{
  for (unsigned idx = 0; idx < n; idx++) out[idx] = ptr[idx];
}

static inline void __convert_half_matrix_to_float(
  const __half* ptr, float* out, size_t m, size_t n, size_t pitch)
{
  for (unsigned i = 0; i < m; i++)
    for (unsigned j = 0; j < n; j++) out[i * n + j] = ptr[i * pitch + j];
}
...
static inline void __convert_float_matrix_to_half(
  const float* ptr, __half* out, size_t m, size_t n, size_t pitch)
{
  for (unsigned i = 0; i < m; i++)
    for (unsigned j = 0; j < n; j++) out[i * pitch + j] = ptr[i * n + j];
}

static void dot_half(const Task* task,
    ...
    case 1: {
      ...
      if (dim1 == 1) {
        ...
        if (partial) {
          ...
          cblas_sgemv(
            CblasRowMajor, CblasTrans, m, n, 1, temp_in2, m, temp_in1, 1, reduce, temp_out, 1);
        ...
        if (partial) {
          ...
          cblas_sgemv(
            CblasRowMajor, CblasNoTrans, m, n, 1, temp_in1, n, temp_in2, 1, reduce, temp_out, 1);
...
template <>
/*static*/ void DotTask<__half>::cpu_variant(const Task* task,
                                             const std::vector<PhysicalRegion>& regions,
                                             Context ctx,
                                             Runtime* runtime)
{
...
#ifdef LEGATE_USE_OPENMP
template <>
/*static*/ void DotTask<__half>::omp_variant(const Task* task,
                                             const std::vector<PhysicalRegion>& regions,
                                             Context ctx,
                                             Runtime* runtime)
{
...
static void dot_float(const Task* task,
                      const std::vector<PhysicalRegion>& regions,
                      Context ctx,
                      Runtime* runtime)
{
...
template <>
/*static*/ void DotTask<float>::cpu_variant(const Task* task,
                                            const std::vector<PhysicalRegion>& regions,
                                            Context ctx,
                                            Runtime* runtime)
{
...
#ifdef LEGATE_USE_OPENMP
template <>
/*static*/ void DotTask<float>::omp_variant(const Task* task,
                                            const std::vector<PhysicalRegion>& regions,
                                            Context ctx,
                                            Runtime* runtime)
{
...
static void dot_double(const Task* task,
                       const std::vector<PhysicalRegion>& regions,
                       Context ctx,
                       Runtime* runtime)
{
...
template <>
/*static*/ void DotTask<double>::cpu_variant(const Task* task,
                                             const std::vector<PhysicalRegion>& regions,
                                             Context ctx,
                                             Runtime* runtime)
{
...
#ifdef LEGATE_USE_OPENMP
template <>
/*static*/ void DotTask<double>::omp_variant(const Task* task,
                                             const std::vector<PhysicalRegion>& regions,
                                             Context ctx,
                                             Runtime* runtime)
{
...
template <typename T>
/*static*/ T DotReducTask<T>::cpu_variant(const Task* task,
                                          const std::vector<PhysicalRegion>& regions,
                                          Context ctx,
                                          Runtime* runtime)
{
...
#ifdef LEGATE_USE_OPENMP
template <typename T>
/*static*/ T DotReducTask<T>::omp_variant(const Task* task,
                                          const std::vector<PhysicalRegion>& regions,
                                          Context ctx,
                                          Runtime* runtime)
{
...
INSTANTIATE_ALL_TASKS(DotReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_DOT) * NUMPY_TYPE_OFFSET +
