...
template<typename T>
/*static*/ void MinTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                        Runtime* runtime) {
...
template<typename T>
/*static*/ void MinTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                        Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
...
template<typename T>
/*static*/ T MinReducTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                          Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim    = derez.unpack_dimension();
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ T MinReducTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                          Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim    = derez.unpack_dimension();
  T                  result = MinReduction<T>::identity;
...
template<typename T>
/*static*/ void BinMinTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                           Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim = derez.unpack_dimension();
  switch (dim) {
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ void BinMinTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                           Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim = derez.unpack_dimension();
  switch (dim) {
...
template<typename T>
/*static*/ void BinMinBroadcast<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim = derez.unpack_dimension();
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ void BinMinBroadcast<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim = derez.unpack_dimension();
  assert(task->futures.size() == 1);
...
template<typename T>
/*static*/ T BinMinScalar<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                          Runtime* runtime) {
...
template<typename T>
/*static*/ void MinRadixTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                             Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ void MinRadixTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                             Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  assert(task->regions.size() <= MAX_REDUCTION_RADIX);
  const int     radix         = derez.unpack_dimension();
...
template<typename T>
/*static*/ T MinScalarTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                           Runtime* runtime) {
  assert(task->futures.size() == 2);
  T       one = task->futures[0].get_result<T>();
...
INSTANTIATE_ALL_TASKS(MinReducTask, static_cast<int>(NumPyOpCode::NUMPY_MIN) * NUMPY_TYPE_OFFSET + NUMPY_REDUCTION_VARIANT_OFFSET)
...
INSTANTIATE_ALL_TASKS(BinMinBroadcast,
                      static_cast<int>(NumPyOpCode::NUMPY_MINIMUM) * NUMPY_TYPE_OFFSET + NUMPY_BROADCAST_VARIANT_OFFSET)
INSTANTIATE_ALL_TASKS(BinMinScalar, static_cast<int>(NumPyOpCode::NUMPY_MINIMUM) * NUMPY_TYPE_OFFSET + NUMPY_SCALAR_VARIANT_OFFSET)
INSTANTIATE_ALL_TASKS(MinRadixTask, static_cast<int>(NumPyOpCode::NUMPY_MIN_RADIX) * NUMPY_TYPE_OFFSET)
INSTANTIATE_ALL_TASKS(MinScalarTask, static_cast<int>(NumPyOpCode::NUMPY_MIN) * NUMPY_TYPE_OFFSET + NUMPY_SCALAR_VARIANT_OFFSET)
