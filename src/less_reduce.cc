...
template <typename T>
/*static*/ bool LessReducTask<T>::cpu_variant(const Task* task,
...
template <typename T>
/*static*/ bool LessReducTask<T>::omp_variant(const Task* task,
                                              const std::vector<PhysicalRegion>& regions,
                                              Context ctx,
                                              Runtime* runtime)
{
...
INSTANTIATE_ALL_TASKS(LessReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_LESS) * NUMPY_TYPE_OFFSET +
...
{
  REGISTER_ALL_TASKS_WITH_BOOL_RETURN(legate::numpy::LessReducTask)
}
}  // namespace
