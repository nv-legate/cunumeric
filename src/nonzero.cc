...
template<typename T>
/*static*/ void CountNonzeroTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                 Runtime* runtime) {
  ...
  switch (dim) {
    ...
    case 2: {
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
          SumReduction<uint64_t>::template fold<true /*exclusive*/>(inout[x][y], static_cast<uint64_t>(in[x][y] != T{0}));
      break;
    }
    case 3: {
      ...
      for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
        for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
          for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++)
            SumReduction<uint64_t>::template fold<true /*exclusive*/>(inout[x][y][z], static_cast<uint64_t>(in[x][y][z] != T{0}));
      break;
    }
    default:
      assert(false);
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
/*static*/ void CountNonzeroTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                 Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
...
namespace detail {
template<typename T>
/*static*/ uint64_t count_nonzero_reduc_helper(const Task* task, const std::vector<PhysicalRegion>& regions,
...
template<typename T>
/*static*/ uint64_t CountNonzeroReducTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                          Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
...
template<typename T>
/*static*/ void CountNonzeroReducWriteTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions,
...
namespace detail {
template<typename T>
/*static*/ uint64_t count_nonzero_reduc_omp_helper(const Task* task, const std::vector<PhysicalRegion>& regions,
                                                   LegateDeserializer& derez) {
  const int dim         = derez.unpack_dimension();
  const int max_threads = omp_get_max_threads();
  uint64_t* results     = static_cast<uint64_t*>(alloca(max_threads * sizeof(uint64_t)));
  for (int i = 0; i < max_threads; i++)
    results[i] = SumReduction<uint64_t>::identity;
  switch (dim) {
    case 1: {
      const Rect<1>          rect = NumPyProjectionFunctor::unpack_shape<1>(task, derez);
      const AccessorRO<T, 1> in   = derez.unpack_accessor_RO<T, 1>(regions[0], rect);
...
#  pragma omp for nowait
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
            SumReduction<uint64_t>::template fold<true /*exclusive*/>(results[tid], in[x][y] != (T)0);
      }
      break;
    }
    case 3: {
...
#  pragma omp for nowait
        for (coord_t x = rect.lo[0]; x <= rect.hi[0]; x++)
          for (coord_t y = rect.lo[1]; y <= rect.hi[1]; y++)
            for (coord_t z = rect.lo[2]; z <= rect.hi[2]; z++)
              SumReduction<uint64_t>::template fold<true /*exclusive*/>(results[tid], in[x][y][z] != (T)0);
      }
      break;
    }
    default:
...
template<typename T>
/*static*/ uint64_t CountNonzeroReducTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                          Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
...
template<typename T>
/*static*/ void CountNonzeroReducWriteTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions,
                                                           Context ctx, Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
...
template<typename T>
/*static*/ void NonzeroTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                            Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
...
#ifdef LEGATE_USE_OPENMP
/*
This implementation has every thread check an equal range of elements for nonzeros and then put them into temporary vectors.  After
that, a partial sum is computed, giving offsets for every partial vector into the final solution.  Finally, every thread copies its
nonzeros into the final vector using the offset computed with the partial sum.

This implementation is potentially susceptible to load imbalance in the final copy (e.g., all nonzeros in one thread).
 */
template<typename T>
/*static*/ void NonzeroTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                            Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          in_dim = derez.unpack_dimension();
  assert(in_dim > 0);
...
#  pragma omp for nowait
        for (coord_t x = in_rect.lo[0]; x <= in_rect.hi[0]; x++) {
          if (in[x] != (T)0) {
            result[tid].push_back(static_cast<uint64_t>(x));
          }
        }
      }
      ...
      std::transform(result.begin(), result.end(), sizes.begin() + 1, [](const auto& vec) { return vec.size(); });
      // prefix sum of sizes
...
template<typename T>
void ConvertRangeToRectTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                            Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
...
#ifdef LEGATE_USE_OPENMP
template<typename T>
void ConvertRangeToRectTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                            Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const coord_t      nonzero_dim = derez.unpack_32bit_int();
  const int          dim         = derez.unpack_dimension();
...
INSTANTIATE_ALL_TASKS(CountNonzeroTask, static_cast<int>(NumPyOpCode::NUMPY_COUNT_NONZERO) * NUMPY_TYPE_OFFSET)
INSTANTIATE_ALL_TASKS(CountNonzeroReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_COUNT_NONZERO) * NUMPY_TYPE_OFFSET + NUMPY_REDUCTION_VARIANT_OFFSET)
INSTANTIATE_ALL_TASKS(CountNonzeroReducWriteTask, static_cast<int>(NumPyOpCode::NUMPY_COUNT_NONZERO_REDUC) * NUMPY_TYPE_OFFSET)
INSTANTIATE_ALL_TASKS(NonzeroTask, static_cast<int>(NumPyOpCode::NUMPY_NONZERO) * NUMPY_TYPE_OFFSET)
INSTANTIATE_INT_TASKS(ConvertRangeToRectTask, static_cast<int>(NumPyOpCode::NUMPY_CONVERT_TO_RECT) * NUMPY_TYPE_OFFSET)
INSTANTIATE_UINT_TASKS(ConvertRangeToRectTask, static_cast<int>(NumPyOpCode::NUMPY_CONVERT_TO_RECT) * NUMPY_TYPE_OFFSET)
