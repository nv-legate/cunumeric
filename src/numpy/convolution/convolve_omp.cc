...
struct ConvolveImplBody<VariantKind::OMP, CODE, DIM> {
                  ...
                  const Rect<DIM>& filter_rect) const
  {
    ...
    for (int d = DIM-1; d >= 0; d--) {
      ...
      total_l2_outputs *=
        ((output_bounds[d] + l2_output_tile[d] - 1) / l2_output_tile[d]);
    }
    ...
    fprintf(stdout,"\n");
    fprintf(stdout,"Total L2 Filter Tiles: %d\n", total_l2_filters);

    fprintf(stdout,"L2 Output Tile: ");
    for (int d = 0; d < DIM; d++)
      fprintf(stdout,"%lld ", l2_output_tile[d]);
    fprintf(stdout,"\n");
    fprintf(stdout,"Total L2 Output Tiles: %d\n", total_l2_outputs);
#endif
    ...
    fprintf(stdout,"\n");
    fprintf(stdout,"Total L1 Filter Tiles: %d\n", total_l1_filters);

    fprintf(stdout,"L1 Output Tile: ");
    for (int d = 0; d < DIM; d++)
      fprintf(stdout,"%lld ", l1_output_tile[d]);
    fprintf(stdout,"\n");
    fprintf(stdout,"Total L1 Output Tiles: %d\n", total_l1_outputs);
    fflush(stdout);
#endif
    ...
    for (unsigned l2_fidx = 0; l2_fidx < total_l2_filters; l2_fidx++) {
      ...
      if (!filter_rect.contains(l2_filter_rect)) {
        ...
        for (int d = 0; d < DIM; d++)
          local_l1_filters *=
            ((l2_filter_rect.hi[d] - l2_filter_rect.lo[d] + l1_filter_tile[d]) /
             l1_filter_tile[d]);
      }
