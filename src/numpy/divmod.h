...
/// Optional constructor from native value
  ...
  uint128_t operator>>(int sh) const
  {
    if (sh == 0) {
      ...
      return uint128_t(
          (hilo_.lo >> sh) | (hilo_.hi << (kPartSize - sh)), (hilo_.hi >> sh));
    }
  }
};
...
struct FastDivmod {
  ...
  template <typename value_t> __CUDA_HD__
  static inline value_t clz(value_t x) {
    for (int i = 31; i >= 0; --i) {
  ...
  template <typename value_t> __CUDA_HD__ 
  static inline value_t find_log2(value_t x) {
    int a = int(31 - clz(x));
   ...
   */
  __CUDA_HD__ 
  static inline void fast_divmod(int& quo, int& rem, int src, int div, unsigned int mul, unsigned int shr) {
#if defined(__CUDA_ARCH__)
  ...
  // For long int input
  __CUDA_HD__ 
  static inline void fast_divmod(int& quo, int64_t& rem, int64_t src, int div, unsigned int mul, unsigned int shr) {
#if defined(__CUDA_ARCH__)
    // Use IMUL.HI if div != 1, else simply copy the source.
    quo = (div != 1) ? __umulhi(src, mul) >> shr : src;
#else
...
struct FastDivmodU64 {
  ...
  inline void operator()(
      uint64_t& quotient, uint64_t& remainder, uint64_t dividend) const
  {
