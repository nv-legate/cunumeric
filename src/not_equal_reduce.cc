...
template<typename T>
/*static*/ bool NotEqualReducTask<T>::cpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                  Runtime* runtime) {
...
template<typename T>
/*static*/ bool NotEqualReducTask<T>::omp_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                  Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim    = derez.unpack_dimension();
  bool               result = true;
...
INSTANTIATE_ALL_TASKS(NotEqualReducTask,
                      static_cast<int>(NumPyOpCode::NUMPY_NOT_EQUAL) * NUMPY_TYPE_OFFSET + NUMPY_REDUCTION_VARIANT_OFFSET)
...
static void __attribute__((constructor)) register_tasks(void) {
  REGISTER_ALL_TASKS_WITH_BOOL_RETURN(legate::numpy::NotEqualReducTask)
}
}    // namespace
