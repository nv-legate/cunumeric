...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_contains_1d(DeferredValue<bool> result,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_contains_2d(DeferredValue<bool> result,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_contains_3d(DeferredValue<bool> result,
...
template <typename T>
/*static*/ DeferredValue<bool> ContainsTask<T>::gpu_variant(
  const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime)
{
  ...
  switch (dim) {
    ...
    case 2: {
      ...
      legate_contains_2d<T>
        <<<blocks, THREADS_PER_BLOCK>>>(result, in, rect.lo, Point<1>(pitch), volume, value);
      break;
    }
    case 3: {
      ...
      legate_contains_3d<T>
        <<<blocks, THREADS_PER_BLOCK>>>(result, in, rect.lo, Point<2>(pitch), volume, value);
      break;
    }
