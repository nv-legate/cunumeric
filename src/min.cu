...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_min_2d(const AccessorRW<T, 2> inout, const AccessorRO<T, 2> in, const Rect<2> bounds, const T identity, const int axis) {
  ...
  if (axis == 0) {
    while (x <= bounds.hi[0]) {
      MinReduction<T>::template fold<true /*exclusive*/>(value, in[x][y]);
      ...
      for (int i = 0; i < warpSize; i++)
        if (same_mask & (1 << i)) {
          if (lowest_index == -1) {
            ...
            MinReduction<T>::template fold<true /*exclusive*/>(value, trampoline[index]);
          }
        }
    }
#endif
  }
  if (value != identity) MinReduction<T>::template fold<false /*exclusive*/>(inout[p], value);
}

template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_min_3d(const AccessorRW<T, 3> inout, const AccessorRO<T, 3> in, const Rect<3> bounds, const T identity, const int axis) {
  ...
  if (axis == 0) {
    while (x <= bounds.hi[0]) {
      MinReduction<T>::template fold<true /*exclusive*/>(value, in[x][y][z]);
...
template<typename T>
/*static*/ void MinTask<T>::gpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                        Runtime* runtime) {
  ...
  const T            initial_value = (task->futures.size() == 1) ? task->futures[0].get_result<T>() : MinReduction<T>::identity;
  switch (init_dim) {
    case 1: {
  ...
  switch (dim) {
    ...
    case 2: {
      ...
      dim3 threads(1, 1, 1);
      dim3 blocks(1, 1, 1);
      raster_2d_reduction(blocks, threads, rect, axis, (const void*)legate_min_2d<T>);
      legate_min_2d<T><<<blocks, threads>>>(inout, in, rect, MinReduction<T>::identity, axis);
      break;
    }
    case 3: {
      ...
      dim3 threads(1, 1, 1);
      dim3 blocks(1, 1, 1);
      raster_3d_reduction(blocks, threads, rect, axis, (const void*)legate_min_3d<T>);
      legate_min_3d<T><<<blocks, threads>>>(inout, in, rect, MinReduction<T>::identity, axis);
      break;
    }
    default:
      assert(false);
...
INSTANTIATE_TASK_VARIANT(MinTask, gpu_variant)

template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_min_reduce_1d(DeferredReduction<MinReduction<T>> result, const AccessorRO<T, 1> in, const size_t iters,
  ...
  for (unsigned idx = 0; idx < iters; idx++) {
    ...
    if (offset < max) {
      ...
      MinReduction<T>::template fold<true>(value, in[x]);
    }
  }
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_min_reduce_2d(DeferredReduction<MinReduction<T>> result, const AccessorRO<T, 2> in, const size_t iters,
  ...
  for (unsigned idx = 0; idx < iters; idx++) {
    ...
    if (offset < max) {
      ...
      MinReduction<T>::template fold<true>(value, in[x][y]);
    }
  }
  reduce_output(result, value);
}
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_min_reduce_3d(DeferredReduction<MinReduction<T>> result, const AccessorRO<T, 3> in, const size_t iters,
  ...
  for (unsigned idx = 0; idx < iters; idx++) {
    ...
    if (offset < max) {
      ...
      MinReduction<T>::template fold<true>(value, in[x][y][z]);
    }
  }
  reduce_output(result, value);
}
...
template<typename T>
/*static*/ DeferredReduction<MinReduction<T>>
    MinReducTask<T>::gpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime) {
  DeferredReduction<MinReduction<T>> result;
  LegateDeserializer                 derez(task->args, task->arglen);
  const int                          dim = derez.unpack_dimension();
  switch (dim) {
    case 1: {
      ...
      if (blocks >= MAX_REDUCTION_CTAS) {
        ...
        legate_min_reduce_1d<T>
            <<<MAX_REDUCTION_CTAS, THREADS_PER_BLOCK>>>(result, in, iters, rect.lo, volume, MinReduction<T>::identity);
      } else {
        legate_min_reduce_1d<T><<<blocks, THREADS_PER_BLOCK>>>(result, in, 1 /*iter*/, rect.lo, volume, MinReduction<T>::identity);
      }
      break;
    }
    case 2: {
      ...
      if (blocks >= MAX_REDUCTION_CTAS) {
        const size_t iters = (blocks + MAX_REDUCTION_CTAS - 1) / MAX_REDUCTION_CTAS;
        legate_min_reduce_2d<T><<<MAX_REDUCTION_CTAS, THREADS_PER_BLOCK>>>(result, in, iters, rect.lo, Point<1>(pitch), volume,
                                                                           MinReduction<T>::identity);
      } else {
        legate_min_reduce_2d<T>
            <<<blocks, THREADS_PER_BLOCK>>>(result, in, 1 /*iter*/, rect.lo, Point<1>(pitch), volume, MinReduction<T>::identity);
      }
      break;
    }
    case 3: {
      ...
      if (blocks >= MAX_REDUCTION_CTAS) {
        const size_t iters = (blocks + MAX_REDUCTION_CTAS - 1) / MAX_REDUCTION_CTAS;
        legate_min_reduce_3d<T><<<MAX_REDUCTION_CTAS, THREADS_PER_BLOCK>>>(result, in, iters, rect.lo, Point<2>(pitch), volume,
                                                                           MinReduction<T>::identity);
      } else {
        legate_min_reduce_3d<T>
            <<<blocks, THREADS_PER_BLOCK>>>(result, in, 1 /*iter*/, rect.lo, Point<2>(pitch), volume, MinReduction<T>::identity);
      }
      break;
    }
    default:
...
INSTANTIATE_DEFERRED_REDUCTION_TASK_VARIANT(MinReducTask, MinReduction, gpu_variant)

template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_1d(const AccessorWO<T, 1> out, const AccessorRO<T, 1> in1, const AccessorRO<T, 1> in2, const Point<1> origin,
                     const size_t max) {
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_1d_inplace(const AccessorRW<T, 1> out, const AccessorRO<T, 1> in, const Point<1> origin, const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x = origin[0] + offset;
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_2d(const AccessorWO<T, 2> out, const AccessorRO<T, 2> in1, const AccessorRO<T, 2> in2, const Point<2> origin,
                     const Point<1> pitch, const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x = origin[0] + offset / pitch[0];
  const coord_t y = origin[1] + offset % pitch[0];
  out[x][y]       = MIN(in1[x][y], in2[x][y]);
}

template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_2d_inplace(const AccessorRW<T, 2> out, const AccessorRO<T, 2> in, const Point<2> origin, const Point<1> pitch,
                             const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x = origin[0] + offset / pitch[0];
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_3d(const AccessorWO<T, 3> out, const AccessorRO<T, 3> in1, const AccessorRO<T, 3> in2, const Point<3> origin,
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_3d_inplace(const AccessorRW<T, 3> out, const AccessorRO<T, 3> in, const Point<3> origin, const Point<2> pitch,
                             const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x = origin[0] + offset / pitch[0];
...
template<typename T>
/*static*/ void BinMinTask<T>::gpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                           Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim = derez.unpack_dimension();
  switch (dim) {
    case 1: {
      ...
      if (task->regions.size() == 2) {
        ...
        const size_t           volume = rect.volume();
        const size_t           blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        legate_binmin_1d_inplace<T><<<blocks, THREADS_PER_BLOCK>>>(out, in, rect.lo, volume);
      } else {
        ...
        const size_t           volume = rect.volume();
        const size_t           blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        legate_binmin_1d<T><<<blocks, THREADS_PER_BLOCK>>>(out, in1, in2, rect.lo, volume);
      }
      break;
    }
    case 2: {
      ...
      if (task->regions.size() == 2) {
        ...
        const size_t           blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        const coord_t          pitch  = rect.hi[1] - rect.lo[1] + 1;
        legate_binmin_2d_inplace<T><<<blocks, THREADS_PER_BLOCK>>>(out, in, rect.lo, Point<1>(pitch), volume);
      } else {
        ...
        const size_t           blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        const coord_t          pitch  = rect.hi[1] - rect.lo[1] + 1;
        legate_binmin_2d<T><<<blocks, THREADS_PER_BLOCK>>>(out, in1, in2, rect.lo, Point<1>(pitch), volume);
      }
      break;
    }
    case 3: {
      ...
      if (task->regions.size() == 2) {
        ...
        const coord_t          diffz    = rect.hi[2] - rect.lo[2] + 1;
        const coord_t          pitch[2] = {diffy * diffz, diffz};
        legate_binmin_3d_inplace<T><<<blocks, THREADS_PER_BLOCK>>>(out, in, rect.lo, Point<2>(pitch), volume);
      } else {
        ...
        const coord_t          diffz    = rect.hi[2] - rect.lo[2] + 1;
        const coord_t          pitch[2] = {diffy * diffz, diffz};
        legate_binmin_3d<T><<<blocks, THREADS_PER_BLOCK>>>(out, in1, in2, rect.lo, Point<2>(pitch), volume);
      }
      break;
    }
    default:
...
INSTANTIATE_TASK_VARIANT(BinMinTask, gpu_variant)

template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_broadcast_1d(const AccessorWO<T, 1> out, const AccessorRO<T, 1> in1, const T in2, const Point<1> origin,
                               const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x = origin[0] + offset;
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_broadcast_1d_inplace(const AccessorRW<T, 1> out, const T in, const Point<1> origin, const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x = origin[0] + offset;
  MinReduction<T>::template fold<true /*exclusive*/>(out[x], in);
}

template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_broadcast_2d(const AccessorWO<T, 2> out, const AccessorRO<T, 2> in1, const T in2, const Point<2> origin,
                               const Point<1> pitch, const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x = origin[0] + offset / pitch[0];
  const coord_t y = origin[1] + offset % pitch[0];
  out[x][y]       = MIN(in1[x][y], in2);
}

template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_broadcast_2d_inplace(const AccessorRW<T, 2> out, const T in, const Point<2> origin, const Point<1> pitch,
                                       const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x = origin[0] + offset / pitch[0];
  const coord_t y = origin[1] + offset % pitch[0];
  MinReduction<T>::template fold<true /*exclusive*/>(out[x][y], in);
}

template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_broadcast_3d(const AccessorWO<T, 3> out, const AccessorRO<T, 3> in1, const T in2, const Point<3> origin,
                               const Point<2> pitch, const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x = origin[0] + offset / pitch[0];
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_binmin_broadcast_3d_inplace(const AccessorRW<T, 3> out, const T in, const Point<3> origin, const Point<2> pitch,
                                       const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x = origin[0] + offset / pitch[0];
  const coord_t y = origin[1] + (offset % pitch[0]) / pitch[1];
  const coord_t z = origin[2] + (offset % pitch[0]) % pitch[1];
  MinReduction<T>::template fold<true /*exclusive*/>(out[x][y][z], in);
}

template<typename T>
/*static*/ void BinMinBroadcast<T>::gpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                                Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  const int          dim = derez.unpack_dimension();
  ...
  switch (dim) {
    case 1: {
      ...
      if (task->regions.size() == 1) {
        ...
        const size_t           volume = rect.volume();
        const size_t           blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        legate_binmin_broadcast_1d_inplace<T><<<blocks, THREADS_PER_BLOCK>>>(out, in2, rect.lo, volume);
      } else {
        const AccessorWO<T, 1> out    = derez.unpack_accessor_WO<T, 1>(regions[0], rect);
        const AccessorRO<T, 1> in1    = derez.unpack_accessor_RO<T, 1>(regions[1], rect);
        const size_t           volume = rect.volume();
        const size_t           blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        legate_binmin_broadcast_1d<T><<<blocks, THREADS_PER_BLOCK>>>(out, in1, in2, rect.lo, volume);
      }
      break;
    }
    case 2: {
      ...
      if (task->regions.size() == 1) {
        ...
        const size_t           blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        const coord_t          pitch  = rect.hi[1] - rect.lo[1] + 1;
        legate_binmin_broadcast_2d_inplace<T><<<blocks, THREADS_PER_BLOCK>>>(out, in2, rect.lo, Point<1>(pitch), volume);
      } else {
        const AccessorWO<T, 2> out    = derez.unpack_accessor_WO<T, 2>(regions[0], rect);
        const AccessorRO<T, 2> in1    = derez.unpack_accessor_RO<T, 2>(regions[1], rect);
        const size_t           volume = rect.volume();
        const size_t           blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
        const coord_t          pitch  = rect.hi[1] - rect.lo[1] + 1;
        legate_binmin_broadcast_2d<T><<<blocks, THREADS_PER_BLOCK>>>(out, in1, in2, rect.lo, Point<1>(pitch), volume);
      }
      break;
    }
    case 3: {
      ...
      if (task->regions.size() == 1) {
        ...
        const coord_t          diffz    = rect.hi[2] - rect.lo[2] + 1;
        const coord_t          pitch[2] = {diffy * diffz, diffz};
        legate_binmin_broadcast_3d_inplace<T><<<blocks, THREADS_PER_BLOCK>>>(out, in2, rect.lo, Point<2>(pitch), volume);
      } else {
        const AccessorWO<T, 3> out      = derez.unpack_accessor_WO<T, 3>(regions[0], rect);
        const AccessorRO<T, 3> in1      = derez.unpack_accessor_RO<T, 3>(regions[1], rect);
        const size_t           volume   = rect.volume();
        ...
        const coord_t          diffz    = rect.hi[2] - rect.lo[2] + 1;
        const coord_t          pitch[2] = {diffy * diffz, diffz};
        legate_binmin_broadcast_3d<T><<<blocks, THREADS_PER_BLOCK>>>(out, in1, in2, rect.lo, Point<2>(pitch), volume);
      }
      break;
    }
    default:
...
INSTANTIATE_TASK_VARIANT(BinMinBroadcast, gpu_variant)
...
template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_min_radix_1d(const AccessorWO<T, 1> out, const MinRadixArgs<T, 1> args, const size_t argmax, const Point<1> origin,
                        const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x   = origin[0] + offset;
  ...
  for (unsigned idx = 1; idx < argmax; idx++)
    MinReduction<T>::template fold<true /*exclusive*/>(val, args.in[idx][x]);
  out[x] = val;
}

template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_min_radix_2d(const AccessorWO<T, 2> out, const MinRadixArgs<T, 2> args, const size_t argmax, const Point<2> origin,
                        const Point<1> pitch, const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x   = origin[0] + offset / pitch[0];
  ...
  T             val = args.in[0][x][y];
  for (unsigned idx = 1; idx < argmax; idx++)
    MinReduction<T>::template fold<true /*exclusive*/>(val, args.in[idx][x][y]);
  out[x][y] = val;
}

template<typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
    legate_min_radix_3d(const AccessorWO<T, 3> out, const MinRadixArgs<T, 3> args, const size_t argmax, const Point<3> origin,
                        const Point<2> pitch, const size_t max) {
  const size_t offset = blockIdx.x * blockDim.x + threadIdx.x;
  if (offset >= max) return;
  const coord_t x   = origin[0] + offset / pitch[0];
  ...
  for (unsigned idx = 1; idx < argmax; idx++)
    MinReduction<T>::template fold<true /*exclusive*/>(val, args.in[idx][x][y][z]);
  out[x][y][z] = val;
}

template<typename T>
/*static*/ void MinRadixTask<T>::gpu_variant(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx,
                                             Runtime* runtime) {
  LegateDeserializer derez(task->args, task->arglen);
  ...
  switch (dim) {
    case 1: {
      ...
      const size_t volume = rect.volume();
      const size_t blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
      legate_min_radix_1d<T><<<blocks, THREADS_PER_BLOCK>>>(out, args, num_inputs, rect.lo, volume);
      break;
    }
    case 2: {
      const Rect<2> rect = NumPyProjectionFunctor::unpack_shape<2>(task, derez);
      ...
      const size_t  blocks = (volume + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
      const coord_t pitch  = rect.hi[1] - rect.lo[1] + 1;
      legate_min_radix_2d<T><<<blocks, THREADS_PER_BLOCK>>>(out, args, num_inputs, rect.lo, Point<1>(pitch), volume);
      break;
    }
    case 3: {
      const Rect<3> rect = NumPyProjectionFunctor::unpack_shape<3>(task, derez);
      ...
      const coord_t diffz    = rect.hi[2] - rect.lo[2] + 1;
      const coord_t pitch[2] = {diffy * diffz, diffz};
      legate_min_radix_3d<T><<<blocks, THREADS_PER_BLOCK>>>(out, args, num_inputs, rect.lo, Point<2>(pitch), volume);
      break;
    }
    default:
      assert(false);
...
INSTANTIATE_TASK_VARIANT(MinRadixTask, gpu_variant)
