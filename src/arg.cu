...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_get_arg_1d(const AccessorWO<int64_t, 1> out,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_get_arg_2d(const AccessorWO<int64_t, 2> out,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_get_arg_3d(const AccessorWO<int64_t, 3> out,
...
template <typename T>
/*static*/ void GetargTask<T>::gpu_variant(const Task* task,
