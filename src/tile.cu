...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_tile_1d(const AccessorWO<T, 1> out,
                 const AccessorRO<T, 1> in,
                 const Point<1> bounds,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_tile_2d_1d(const AccessorWO<T, 2> out,
                    const AccessorRO<T, 1> in,
                    const Point<1> bounds,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_tile_2d_2d(const AccessorWO<T, 2> out,
                    const AccessorRO<T, 2> in,
                    const Point<2> bounds,
                    const Point<2> origin,
                    const Point<1> pitch,
                    const size_t max)
{
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_tile_3d_1d(const AccessorWO<T, 3> out,
                    const AccessorRO<T, 1> in,
                    const Point<1> bounds,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_tile_3d_2d(const AccessorWO<T, 3> out,
                    const AccessorRO<T, 2> in,
                    const Point<2> bounds,
                    const Point<3> origin,
                    const Point<2> pitch,
...
template <typename T>
__global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  legate_tile_3d_3d(const AccessorWO<T, 3> out,
                    const AccessorRO<T, 3> in,
                    const Point<3> bounds,
                    const Point<3> origin,
                    const Point<2> pitch,
                    const size_t max)
{
...
template <typename T>
/*static*/ void TileTask<T>::gpu_variant(const Task* task,
                                         ...
                                         Runtime* runtime)
{
  ...
  switch (dst_dim) {
    case 1: {
      ...
      const AccessorRO<T, 1> in  = derez.unpack_accessor_RO<T, 1>(
        regions[1], Rect<1>(Point<1>::ZEROES(), bounds - Point<1>::ONES()));
      ...
      switch (src_dim) {
        case 1: {
          ...
          const AccessorRO<T, 1> in = derez.unpack_accessor_RO<T, 1>(
            regions[1], Rect<1>(Point<1>::ZEROES(), bounds - Point<1>::ONES()));
          legate_tile_2d_1d<T>
            <<<blocks, THREADS_PER_BLOCK>>>(out, in, bounds, rect.lo, pitch, volume);
          break;
        }
        case 2: {
          const Point<2> bounds     = derez.unpack_point<2>();
          const AccessorRO<T, 2> in = derez.unpack_accessor_RO<T, 2>(
            regions[1], Rect<2>(Point<2>::ZEROES(), bounds - Point<2>::ONES()));
          legate_tile_2d_2d<T>
            <<<blocks, THREADS_PER_BLOCK>>>(out, in, bounds, rect.lo, pitch, volume);
          break;
        }
      ...
      switch (src_dim) {
        case 1: {
          ...
          const AccessorRO<T, 1> in = derez.unpack_accessor_RO<T, 1>(
            regions[1], Rect<1>(Point<1>::ZEROES(), bounds - Point<1>::ONES()));
          legate_tile_3d_1d<T>
            <<<blocks, THREADS_PER_BLOCK>>>(out, in, bounds, rect.lo, Point<2>(pitch), volume);
          break;
        }
        case 2: {
          const Point<2> bounds     = derez.unpack_point<2>();
        ...
        case 3: {
          ...
          const AccessorRO<T, 3> in = derez.unpack_accessor_RO<T, 3>(
            regions[1], Rect<3>(Point<3>::ZEROES(), bounds - Point<3>::ONES()));
          legate_tile_3d_3d<T>
            <<<blocks, THREADS_PER_BLOCK>>>(out, in, bounds, rect.lo, Point<2>(pitch), volume);
          break;
        }
